...
- Фоновое обновление карточки покрывают `CatalogTitleBackgroundRefreshTest`, `RefreshSeasonvarCatalogTitleJobTest` и `CatalogTitleLiveRefreshTest`: atomic dispatch/freshness, unique/group locks, targeted pipeline, sanitized failure payload, трёхсекундный visible polling, полный rerender и сохранение валидного player selection. Существующий HTTP-fake тест `SeasonvarParsePageCommandTest::test_it_parses_requested_page_and_all_detected_seasons_into_one_title` остаётся all-seasons регрессией.
