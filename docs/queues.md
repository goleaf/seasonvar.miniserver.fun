...
- Queued finalizer выбирает `licensed_media` только по наступившему `next_check_at`, выполняет bounded source probes вне транзакции каталога и применяет health state короткой retry-aware transaction. После batch он пересобирает рекомендации и stats snapshot; permanent source error не делает сам finalizer job failed.
