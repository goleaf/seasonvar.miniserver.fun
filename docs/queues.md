...
- Cache-warm worker устанавливается из `deploy/systemd/seasonvar-cache-warm-worker.service`; общий импорт слушает только `seasonvar-import` через `seasonvar-import-worker@.service`, а открытая пользователем карточка обрабатывается отдельным приоритетным пулом `seasonvar-title-refresh-worker@.service`. Рекомендуемый стартовый размер IO-bound пула — 32 процесса (`seasonvar-title-refresh-worker@{1..32}.service`), но это только доступная мощность deployment, без application-level limit: число известных страниц тайтла не обрезается и пул можно расширять без изменения кода. Разные `catalog_title_id` выполняются независимо, а unique/lock одного тайтла не блокирует обновление остальных.
