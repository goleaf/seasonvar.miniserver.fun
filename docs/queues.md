...
- Cache-warm worker устанавливается из `deploy/systemd/seasonvar-cache-warm-worker.service`; import workers — из `seasonvar-import-worker@.service`. Import worker слушает `seasonvar-title-refresh,seasonvar-import` именно в таком порядке, чтобы открытая пользователем карточка не ждала за всем catalog-wide backlog. Production запускает пул минимум из пяти экземпляров шаблона: разные `catalog_title_id` выполняются независимо, а unique/lock одного тайтла не блокирует обновление остальных.
