...
- `SeasonvarGlobalImportRunCoordinator` является общей single-flight границей для admin, CLI/cron и retry: повторный глобальный вызов переиспользует active `queued/running` sitemap run и не dispatches page work второй раз. Targeted title refresh остаётся независимым.
...
- Уникальный `WakeSeasonvarImportFinalizers` scheduled каждые 10 минут через `redis-locks`, `withoutOverlapping` и `onOneServer`. Он bounded-пачкой повторяет сигналы только для counter-ready групп и active global runs; default `SEASONVAR_QUEUE_FINALIZER_WATCHDOG_BATCH_SIZE=250`. Watchdog не очищает и не retry-ит failed jobs.
