...
| Queue | production Redis, `after_commit=true`; synchronous fallbacks exist where documented | verified current host boundary | Thirteen application job classes and ten notification classes were inventoried. Class/constructor changes require pending-job compatibility and worker restart. `TD-012` aligns only the exact configured cache-warm connection/queue lease with `max(base, timeout + 60)`; payloads, serializers, keys and queue rows are unchanged, while other pools retain the base lease. Production evidence after graceful recycle retained heartbeat beyond the former 120-second boundary and exposed an actual 643-second remaining TTL immediately after the next job began. |
...
| Chromium | Task 29 headless desktop/mobile smoke on the configured HTTPS host | partially verified | 19 desktop and 6 mobile journeys covering home, catalogue, representative title/player shell, search, calendar, requests, help, Premium, RU/EN authentication pages and guest private/admin redirects rendered without horizontal overflow, raw keys, console/page errors or service-worker registration. Historical `/` `39.5 s` and first mobile `/titles` `52.0 s` observations remain non-SLA evidence in `TD-011`; a 20.07.2026 server-side follow-up removed four confirmed correlated homepage count queries and reduced the same direct builder probe from `19 864,69 ms` to `1 355,92 ms`, but did not repeat the complete browser/device matrix or close catalogue latency. Authenticated mutations, real media playback, external providers and non-Chromium devices were not exercised. |
...
Последующий maintenance follow-up 20.07.2026 закрыл `TD-012`: после безопасного завершения активной job и graceful recycle новый worker продолжал `WarmCatalogCaches` за прежней 120-секундной границей, сохраняя heartbeat с оставшимися 525 секундами. Следующая job начала 660-секундный lease; read-only TTL сразу после старта составлял 643 секунды. `app:health --json` вернул `ready=true`, а cache-warm отображался как `degraded` только из-за фактического backlog, не как остановленный. Unit-контракт отдельно подтверждает прежнее быстрое истечение base pool и bounded expiry cache-warm после timeout/grace. Memcached остаётся независимым открытым `TD-010`.
...
- Исторический снимок до исправления показал ложный `failed` cache-warm после 120 секунд. Follow-up после graceful recycle подтвердил новый bounded lease: при 1 171 pending и одной reserved job, возрасте процесса более 136 секунд и продолжающейся `WarmCatalogCaches` heartbeat оставался доступен с TTL 525 секунд, поэтому pool честно сообщал `degraded`, а import/title-refresh — `ok`. После завершения job следующий heartbeat имел 643 секунды TTL при configured lease 660. Memcached остаётся недоступен. Queue deletion/retry/rewrite, случайное масштабирование и принудительное прерывание job не выполнялись.
...
## GitHub Actions compatibility от 20.07.2026
...
Repository settings read-back подтвердил `allowed_actions=selected`, обязательный full SHA, GitHub-owned actions и единственный точный внешний `setup-php` ref. Закреплённый commit соответствует подписанному выпуску `setup-php 2.37.2`; новый action/package/runtime upgrade не потребовался. Ruleset `Protect main history` запрещает удаление и non-fast-forward update `main`, secret scanning/push protection сохранены, passive Dependabot alerts включены и показали `0` открытых alerts на момент проверки. Ручной run [№223](https://github.com/goleaf/seasonvar.miniserver.fun/actions/runs/29712616978) под этой политикой полностью завершил `Backend`, `Frontend` и `Browser`. GitHub/registry outage, новый advisory и реальная application regression остаются честными отказами; remote configuration rollback описан в `CI-R-002`.
