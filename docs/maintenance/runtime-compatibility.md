...
- Финальный `app:deployment-check --json` после внешних batches 32–33 завершился `ready`: production environment/debug/logging, все 110 migrations, SQLite quick/FK, required indexes, FTS `32929/32929` и cache transports прошли. Integrity check занял `252966 ms` под активной нагрузкой; warnings по `32771` historical failed jobs и неподтверждённому отдельному forever-importer process требуют ручной disposition, но не скрыты как successful automation.
- `app:health --json` отдельно остаётся `ready=true`/`degraded`: database и все критические Redis roles доступны, import/title-refresh workers отвечают, но Memcached недоступен, а `cache-warm-v2` после restart имеет delayed work без свежего heartbeat. На хосте фактически присутствуют четыре import-, восемь title-refresh- и один cache-warm worker; queue deletion/retry или масштабирование наугад не выполнялись. Readiness и health не подменяют друг друга.
