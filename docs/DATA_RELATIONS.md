...
Фильтры Top 100 не создают новых связей: диапазон читает индексированное `catalog_titles.year`, страна использует существующие `countries.slug` и `catalog_title_country`. Form Request допускает только существующий canonical slug, DTO передаёт условия в общий eligibility query до ранжирования и лимита; UI получает не более 100 упорядоченных country options. Query-вариации разделяются нормализованным cache dimension, но не становятся отдельными индексируемыми сущностями или строками схемы.
