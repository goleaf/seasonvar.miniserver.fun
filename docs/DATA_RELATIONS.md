...
Title merge reconciles the complete canonical row. Watchlist remains monotonic OR; rating follows version then trustworthy activity comparison; feedback uses `blacklisted > not_interested`; status uses `dropped > completed > watching > planned`; every version remains max-monotonic. Semantic timestamps remain semantic: technical `updated_at` may resolve an equal legacy conflict but is not copied into a semantic column. This prevents duplicate consolidation from losing a blacklist/not-interested/dropped decision.
