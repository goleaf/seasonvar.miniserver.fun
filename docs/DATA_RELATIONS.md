...
- Удаление account через общую web/API boundary внутри одной транзакции явно удаляет personal access tokens, password-reset rows, database sessions и owner morph `notifications`, затем `User`. FK `cascadeOnDelete` удаляет `catalog_title_user_states`, `episode_view_progress`, preferences и другие owner rows; community/request/ticket/payment evidence следует explicit set-null/domain cleanup. `admin_audit_events.actor_id` — единственный user FK с `RESTRICT`, поэтому административный actor получает fail-closed retention validation до mutation. `CatalogTitle`, seasons/episodes и импортные metadata не принадлежат пользователю и сохраняются.
...
- `audience=public` доступна гостю, `audience=authenticated` — только текущему `User`. Оба SQL scope и решение для загруженной модели формирует `CatalogEntitlementService`. Household/детские playback-профили, PIN, роли entitlement, подписки, покупки, trial, территориальные лицензии и concurrent streams отсутствуют и не симулируются; существующий публичный `UserProfile` не меняет доступ к видео, а player preferences хранятся только в канонических account settings.
...
| --- | --- |
| `catalog_collections` | Stable DB ID, unique public UUID и global current slug; nullable owner/category FK, original name/description, enum-backed type/visibility/moderation/sort, `mode=manual|smart`, nullable versioned `smart_rules`, optional content locale, feature/publication state, content version, timestamps и soft delete. Собственных image metadata нет. |
...
| `catalog_collection_items` | Одна manual serial membership: collection FK, `CatalogTitle` FK, nullable added-by FK, manual position и timestamps. Unique collection/title запрещает логический дубль; title/collection и collection/position indexes обслуживают discovery и ordering. Smart mode эти строки не создаёт и не читает как состав. |
...
- `mode=manual` остаётся default для всех прежних строк. `mode=smart` допустим только вместе с `type=user`, owner, `visibility=private`, nullable category и непустым нормализованным `smart_rules_version=1`; смена mode существующей записи и manual membership mutations для smart mode запрещены. Unknown rule key/version fail-closed и не превращается в SQL.
...
- Исключение для восстановления прежней публикации является отдельной
  ручной границей: immutable exact `provider + source_key` allowlist может
  назначить только заранее проверенный stable child slug десяти
  source-managed rows. Эта DML-reconciliation не запускается importer,
  scheduler или migration, не принимает произвольные ID/URL/названия и не
  ослабляет nullable category для остальных trusted source rows.
...
- Manual positions последовательны после remove/move/reorder/merge. `moveWithinWindow` принимает один canonical item ID и absolute target, но под collection lock разрешает current/target только внутри одного bounded pagination window и обновляет затронутый диапазон; browser не передаёт collection ID, произвольный столбец или полный порядок. Automatic ordering не меняет manual positions. Стабильный secondary key — item/title ID в зависимости от sort.
- Smart result является динамической owner-scoped проекцией `CatalogTitle`: taxonomy/rating/season/media predicates комбинируются с user-state/progress/release predicates одним SQL builder. Сохранённого снимка, denormalized count, cache row, background rebuild или second membership relation нет; пагинация и deterministic title-ID tie-breaker ограничивают hydration.
...
- Текущий editorial dashboard/editor всегда создаёт и изменяет `catalog_collection_translations.locale=ru`; RU/EN selector и подпись языка отсутствуют. Существующие строки с другими locale остаются доступными read/fallback contract и не удаляются при русском сохранении. User collection content locale не меняется этим редакторским правилом.
- Remote source не является identity тайтла. Membership появляется только после точного indexed match по primary/original/approved-alias normalized key с отсутствующим hard conflict года/типа; ambiguous/unmatched rows не получают `catalog_title_id`. Complete run удаляет отсутствующие source-owned membership/signal rows, partial run сохраняет прежнее состояние.
- `editorial_collection:{source_id}` хранится в существующем recommendation signal domain только для текущих matched source items. Его вес — profile input, а не stored recommendation edge; affected title IDs от добавления и удаления помечаются dirty для scoped rebuild.
...
- Для smart mode новый collection index не добавлен: owner dashboard продолжает использовать `catalog_collections_owner_order_idx`, а rule query опирается на существующие slug/pivot, provider-rating, season-publication, owner/title state/progress, release и media-publication indexes. Disposable SQLite `EXPLAIN QUERY PLAN` подтвердил indexed searches и отсутствие global `SCAN seasons`; correlated grouped season predicate заменил первоначальный scan-shaped aggregate.
...
Category migrations additive и SQLite-compatible: сначала создают dictionary/translations, затем nullable FK/index и детерминированные 5 root + 31 child с `ru|en` rows; существующие 1 403 подборки намеренно не классифицируются migration. Отдельная проверенная production reconciliation может назначить категории десяти exact source-managed rows, но не является backfill остальных записей. После подтверждённой exact-prefix очистки migration `2026_07_25_140300` удаляет пять collection и три source cover columns. Её `down()` может вернуть только пустые nullable/default columns, но не удалённые metadata/файлы; media rollback невозможен по продуктовому требованию. Migration `2026_07_26_232000` additive добавляет `mode`, nullable JSON rules и version с manual default без DML/index/backfill; `down()` удаляет только эти три колонки. После появления smart rows безопаснее forward-fix, потому что rollback колонок удалил бы пользовательские правила. Watchlist, rating, progress, history, comments, reviews, catalog identity, title posters и profile uploads не затрагиваются.
...
- `comments_target_public_count_idx(target_type,target_id,status,deleted_at)` обслуживает exact authoritative public count. Он добавлен отдельно после production-style измерения: SQLite выбирал глобальный moderation index и тратил около 4,9 секунды на цель из 139 строк, тогда как exact target prefix возвращал тот же aggregate за миллисекунды. Индекс не дублирует root-list ordering, потому что public count не ограничивает `parent_id` и включает roots/replies.
- `comments_author_activity_idx(user_id,status,deleted_at,created_at,id)` обслуживает private activity и Task 14 public-profile author prefix; public page/count дополняют его тем же published/non-deleted predicate и viewer-visible `catalog_title_id` subquery, не вводя отдельный aggregate или duplicate profile index. `comments_duplicate_window_idx(user_id,target_type,target_id,parent_id,body_hash,created_at)` обслуживает short same-thread duplicate detection; `comments_title_cache_idx(catalog_title_id,status,deleted_at)` — targeted invalidation/account identity changes. Reaction unique/totals indexes обслуживают mutation и public aggregates, `comment_reactions_user_current_idx(user_id,comment_id,type)` — один grouped viewer overlay, а отдельный user/created/id composite остаётся для private export ordering.
...
Migrations `2026_07_15_210000`–`210300`, focused relationship index `2026_07_15_235200`, removed-state convergence `2026_07_18_193800` и public-count index `2026_07_18_193801` additive и SQLite-compatible. Они не переписывают `catalog_title_reviews`, каталог, collections, watchlist/rating, progress/history, media или import state. Повторный audit 3 707 221 comments/9 265 514 reactions не нашёл competing tables, orphan/circular/cross-target/deep relations, translated statuses, duplicate reactions/submission keys или invalid enum rows. Единственная демонстрационная `removed` row без deletion metadata исправляется canonical `DemoCommunityStage` при штатном идемпотентном повторе; `DemoDataAuditor` теперь отклоняет такой state и cross-target/deep demo replies. `193800` idempotently заполняет только отсутствующие reason/actor/timestamp у уже stable `removed` rows через прежние moderation/update/create evidence, не меняя body/ID/thread/reaction/report. Его `down()` намеренно не воссоздаёт invalid null evidence; rollback code/index не требует отката этого data repair. `CommentSchema` отдельно проверяет canonical row и engagement/relationship/notification capabilities; feature flag влияет только на writable UI и не пропускает lifecycle/privacy работу над существующей схемой. Permanent collection retirement bulk-обновляет и активные rows, и tombstones до stable `removed/privacy`, сохраняет существующий `deleted_at` и всю thread/report evidence. После появления production discussion data rollback базовых tables допустим только после private export/backup; `down()` не переносит данные в reviews.
...
Read-only повторный аудит 18.07.2026 подтвердил `1 720 085` review rows (`73 184` provider и `1 646 901` user), `3 294 158` helpfulness votes, `79` aliases и ноль duplicate non-null ownership/submission keys, duplicate `(review,user)` votes, self votes, orphan title/user/vote targets, invalid rating/vote/origin/status values или duplicate current user/title reviews. Единственная user row со stable `status=removed` не имела deletion metadata; она оставалась публично недоступной, но нарушала lifecycle evidence. `2026_07_18_220000_repair_removed_review_deletion_state.php` idempotently заполняет только отсутствующие moderator reason/actor/timestamp из уже сохранённых moderation/update/create timestamps. Migration не меняет body, ID, title, ownership, rating, votes, reports или aliases; её `down()` намеренно не воссоздаёт недостоверные `NULL`. Новые uniqueness и performance indexes не нужны: существующие public-list, author-activity, user/title rating и vote-totals indexes выбраны фактическими `EXPLAIN QUERY PLAN`.
...
- `tag_translations(tag_id, locale)` — localized label/plain-text description/SEO; `tag_aliases(locale, normalized_hash)` — exact alternate identity и optional redirect slug, причём write boundary не допускает один normalized alias у разных canonical targets даже между locale; `tag_slugs(slug)` — current-slug history; `tag_synonyms(tag_id, related_tag_id, relationship)` — bounded semantic relation; `tag_merge_events` — append-only source/target/actor/impact snapshot.
...
- `user_tags` — отдельный private owner aggregate со UUID, original Unicode name, normalized hash, optional explicitly known content locale, version, timestamps/soft delete. Web create не показывает language control и пишет `content_locale=null`, потому что interface locale не доказывает язык UGC; edit сохраняет прежнее explicit значение, а API меняет его только при наличии nullable allowlisted поля. `catalog_title_user_tag` связывает запись только с `CatalogTitle` и хранит deterministic `position`; user ID не принимается из request. Season/Episode/Collection/Comment polymorphic tag pivots отсутствуют.
...
- Tag administration предоставляет allowlisted locale selector для `tag_translations` и `tag_aliases`, сохраняет `ru|en` отдельно и не смешивает display label со stable identity. Translation edit передаёт общий optimistic `content_version`; alias конфликт проверяется между всеми locale и canonical targets. Existing rows сохраняются, отсутствие перевода продолжает использовать active/fallback/base read contract.
...
- Existing-user backfill идемпотентно создаёт collision-safe usernames. Он не меняет users, collections, comments, reviews, library/progress/history или privacy choices; new registration получает private model defaults. Account name change централизованно increment-ит `content_version`, инвалидирует old/current profile summary и search suggestions для Livewire/API. Account deletion cascade-removes profile/aliases and nulls report lifecycle FKs after private media purge; cache/search invalidation планируется after commit.
...
- Profile media rows после upload/demo repair всегда указывают на responder-compatible owned path и WebP derivative design size: avatar `320×320`, cover `1280×360`. Исходный filename/metadata не являются данными профиля.
- Read-only повторный аудит 19.07.2026 подтвердил `102` users и `102` profiles, zero missing/orphan/duplicate/invalid identity, history, privacy, moderation или media rows, `77` public и `25` private active profiles. Existing unique/current/history/report/block/mute indexes покрывают реальные lookup/queue predicates; дополнительная схема и speculative index не потребовались.
...
Фильтры Top 100 не создают новых связей: диапазон читает индексированное `catalog_titles.year`, страна использует существующие `countries.slug` и `catalog_title_country`, жанр — `genres.slug` и `catalog_title_genre`. Form Request допускает только существующие canonical slug, DTO передаёт условия в общий eligibility query до ранжирования и лимита; UI получает не более 100 упорядоченных вариантов каждого справочника. Query-вариации разделяются нормализованным cache dimension, но не становятся отдельными индексируемыми сущностями или строками схемы.
...
Additive migration `2026_07_17_170000_create_release_calendar_domain.php` добавляет `release_schedule_entries`, append-style `release_schedule_corrections`, unique user/title `release_calendar_subscriptions` и owner preferences. Stable UUID/`logical_key` не зависят от title/date/locale; original, translation, subtitle и portal publication остаются разными event types. Partial precision хранится без fake timestamp, manual lock защищает редакционную дату от observer/import, а merge переносит цели и объединяет подписки без удаления истории. Индексы обслуживают bounded time/status/type/target queries и timelines; public/private visibility продолжает принадлежать существующим catalog scopes. Полный contract и rollback — в [`release-calendar.md`](release-calendar.md).

Additive reversible migration `2026_07_25_220709_create_release_calendar_feeds_table.php` добавляет owner-scoped `release_calendar_feeds`. `public_id` является management identity, `token_hash` — unique SHA-256 lookup, а восстанавливаемый `token_secret` хранится только через Laravel encrypted cast и скрыт из model serialization/account export. Stable `scope` принимает `all|collection|new_episodes|season_premieres|title|translation|subtitles`; nullable collection/title/language/translation поля допустимы только по server-side матрице scope. User hard delete отзывает все feeds, collection/title hard delete использует cascade, soft delete закрывает выдачу, а title merge переносит target. Индекс `(user_id,created_at,id)` обслуживает bounded owner list, unique token hash — capability lookup.

Эта же migration добавляет `(is_public,starts_at,id)` и `(is_public,date_value,id,date_end)` для status-free ICS window. Они не дублируют существующие status-prefixed public indexes: feed сохраняет cancelled/tentative state и поэтому не фильтрует один status. На disposable SQLite `EXPLAIN QUERY PLAN` выбрал новые time/date/token lookup indexes; порядок `date_value,id` устранил temporary sort в изолированном date-range плане, а trailing `date_end` сохранил covering overlap check. Migration не выполняет backfill или production DML.
...
Migration `2026_07_16_240000_create_catalog_recommendation_shadow_builds.php` additive создаёт `catalog_recommendation_builds` и owned `catalog_recommendation_build_rows`. Build хранит algorithm/feature version, lifecycle, bounded metrics/failure и timestamps; `updated_at` обновляется как heartbeat на bounded profile/scoring chunks, а `building` старше `recommendations.similarity_v6.stale_build_minutes` без heartbeat атомарно становится `failed`. Rows повторяют только public recommendation payload. Unique `(build_id,catalog_title_id,recommended_title_id)` исключает пару-дубль, source/rank и candidate/score indexes обслуживают quality evaluation и атомарную `insertUsing` activation. Active `catalog_title_recommendations` не очищается до gate pass и всегда сохраняет связанный active build; terminal diagnostic history после обычного завершения или аварии активации ограничивается последними `recommendations.similarity_v6.build_history_limit` build'ами, а удаление более старого owner каскадно удаляет только его shadow rows. Rollback shadow schema не удаляет текущую выдачу.

Migration `2026_07_16_240200_add_score_distribution_index_to_catalog_recommendation_build_rows.php` additive и reversible добавляет covering index `(build_id,score,id)` для упорядоченного чтения распределения score внутри одной теневой сборки. Он устраняет временную B-tree SQLite у quality gate; прежние source/rank и candidate/score indexes сохраняют свои отдельные lookup-контракты.

Migration `2026_07_16_240100_create_catalog_recommendation_dirty_titles.php` additive создаёт unique `catalog_title_id` queue row с bounded reason и `marked_at`. FK cascade удаляет только orphan queue state. Индекс `(marked_at,id)` обслуживает bounded oldest-first batch; повторная отметка использует upsert и не создаёт дубль. Queue очищается только после успешной scoped transaction или активированной full shadow build. Collection-sync job запрещает full fallback: при несовместимой active version или слишком широком affected set она возвращает deferred result и сохраняет queue для полного импортного цикла.

`catalog_title_recommendation_signals` больше не хранит производные taxonomy/rating/year/page-quality копии для новых Seasonvar parses. Допустимый runtime reader ограничен `provider_recommendation|related_title`; legacy generic `seasonvar_info` rows удаляет отдельный allowlisted `chunkById(1000)` только после подтверждённой v6 activation. Другие sources и provider relation types сохраняются.

Migration `2026_07_16_120000_add_canonical_recommendation_discovery.php` additive и не меняет `catalog_title_recommendations`, watchlist, rating или progress. Она добавляет nullable `recommendation_feedback`, feedback version/timestamp, nullable `watch_status` и watch-status version в existing unique `(user_id,catalog_title_id)` row. Stable values: feedback `more_like_this|not_interested|blacklisted`, status `planned|watching|completed|dropped`; translated labels не хранятся. `more_like_this` является положительным источником, а не скрытием; только два отрицательных feedback values входят в hidden list и notification suppression.

Migration `2026_07_16_230000_add_recommendation_signal_timestamps.php` additive добавляет nullable `watchlist_updated_at`, `rating_updated_at` и `watch_status_updated_at` в ту же canonical row. Backfill намеренно отсутствует: shared technical `updated_at` не доказывает, какой private signal изменился, поэтому legacy values сохраняются с `null` provenance до следующей фактической mutation соответствующего поля. Поля внутренние и не добавлены в API/export/public cache. `down()` возвращает прежние definitions двух заменённых индексов, затем удаляет только три semantic timestamp column.

Migration `2026_07_16_230100_add_recommendation_signal_owner_indexes.php` не меняет данные или public activity indexes. Она добавляет три reversible owner-first covering index точно для bounded personalized source windows; это отдельный query contract от library lists, которые по-прежнему используют technical update ordering для UI state, а не для recommendation provenance.

Migration `2026_07_26_120000_replace_recommendation_feedback_index_for_activity_order.php`
reversible заменяет прежний feedback index тем же именем и не переписывает
строки. Новый порядок
`(user_id,recommendation_feedback,recommendation_feedback_updated_at,id,catalog_title_id)`
обслуживает bounded positive-feedback source window, exact negative filters
...
Title merge reconciles the complete canonical row. Watchlist remains monotonic OR; rating follows version then trustworthy activity comparison; feedback uses `blacklisted > not_interested > more_like_this`; status uses `dropped > completed > watching > planned`; every version remains max-monotonic. Semantic timestamps remain semantic: technical `updated_at` may resolve an equal legacy conflict but is not copied into a semantic column. This prevents duplicate consolidation from losing a blacklist/not-interested/dropped decision.

Upcoming discovery first builds bounded title-ID pools from canonically available future episodes and visible future title years, then reapplies the shared title visibility/filter/order query. Draft/deleted/unavailable episode or season rows cannot supply `next_release_at`; an empty calendar does not run one correlated release scan per catalogue title.
...
Uniqueness обеспечивает public UUID/number, idempotent submission, один active exact identity, один confirmation/follow/occurrence на user/ticket и один mapping на merged duplicate. 41 явный index обслуживает requester/support pagination, type/target/source/error candidate narrowing, assignment queue, ordered message/attachment/status/occurrence/redaction timelines и participant lookups; отдельный `(requester_id, updated_at, id)` покрывает основной requester sort. Additive `2026_07_17_141321_add_sort_ranks_to_technical_issues_table.php` сохраняет stable text codes, backfill-ит только derived `severity_sort_rank`/`priority_sort_rank` и добавляет `(sort_rank, created_at, id)` для обеих deterministic support-сортировок, устраняя full queue scan и temporary B-tree. Attachment bytes и message bodies в list queries не загружаются. Foreign keys используют cascade только для owned child evidence, `nullOnDelete` для retained actor/catalog links и не удаляют catalog/media records.
...
Additive migration `2026_07_18_070229_create_premium_domain_tables.php` создаёт plans, promotions/coupons/redemptions, checkouts, subscriptions, payments/refunds/disputes, entitlements, provider events и immutable audit. Stable public/code/provider identities и `application_key` имеют unique constraints; resolver query использует `(user_id,feature_code,revoked_at,ends_at)`, owner histories — user/status/time, provider retry — event status/time. Payment/subscription rows могут сохранить историю после удаления user через nullable FK, а grants и redemptions следуют account lifecycle.

Entitlement источники не перезаписываются: lifetime — `ends_at=null` плюс `is_lifetime`, fixed grant продлевается от позднего current expiry/now, provider subscription period сохраняется точно, refund/chargeback затрагивает только строки связанного payment. Полная схема, index rationale, normalization и rollback boundary принадлежат [`premium.md`](premium.md).

## Help-center relations и integrity

`help_articles` и `help_categories` владеют стабильными UUID/code; locale rows принадлежат base identity, а current/history slug разделены. Уникальны translation на `(owner, locale)`, current/history slug на `(locale, slug)`, alias на `(article, locale, normalized_alias)`, directed related/context relation и actor feedback. Category parent ограничен одним дочерним уровнем action validation; replacement graph проверяется на циклы.

Revisions append-only и не загружаются public query. Feedback/report сохраняют article и exact translation identity; user FK nullable для account deletion, HMAC actor/dedupe не раскрывается. `technical_issues.help_article_id` — nullable context relation, а не копия статьи или private ticket data. Queries/index rationale и rollback описаны в [`help-center.md`](help-center.md).

## Playback identity и progress Task 07
...
| --- | --- |
| `catalog_title_user_states` | Одна unique `(user_id,catalog_title_id)` запись объединяет bookmark/favorite `in_watchlist`, rating, serial status, recommendation feedback и независимые semantic timestamps. Bookmark не означает `planned`; `more_like_this`, `not_interested` и `blacklisted` не удаляют bookmark/status/progress/collection membership, а скрывают только два отрицательных значения. |
| `episode_view_progress` | Одна unique `(user_id,episode_id)` запись остаётся источником automatic position/history/completion. Nullable `completion_source=playback|manual|anonymous` различает provenance без копии progress. `anonymous` означает bounded verified-account import из browser snapshot: он не создаёт completion/verified-watching, не содержит playback session/media и заменяется trusted playback provenance при первом настоящем событии. Title merge применяет precedence `manual > playback/legacy playback session > anonymous > none`, поэтому imported state не вытесняет stronger evidence. Manual unwatch снимает только manual completion; automatic completion и позиция сохраняются. |
