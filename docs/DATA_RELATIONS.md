...
Migration `2026_07_16_240000_create_catalog_recommendation_shadow_builds.php` additive создаёт `catalog_recommendation_builds` и owned `catalog_recommendation_build_rows`. Build хранит algorithm/feature version, lifecycle, bounded metrics/failure и timestamps; rows повторяют только public recommendation payload. Unique `(build_id,catalog_title_id,recommended_title_id)` исключает пару-дубль, source/rank и candidate/score indexes обслуживают quality evaluation и атомарную `insertUsing` activation. Active `catalog_title_recommendations` не очищается до gate pass и всегда сохраняет связанный active build; terminal diagnostic history после обычного завершения или аварии активации ограничивается последними `recommendations.similarity_v6.build_history_limit` build'ами, а удаление более старого owner каскадно удаляет только его shadow rows. Rollback shadow schema не удаляет текущую выдачу.

Migration `2026_07_16_240100_create_catalog_recommendation_dirty_titles.php` additive создаёт unique `catalog_title_id` queue row с bounded reason и `marked_at`. FK cascade удаляет только orphan queue state. Индекс `(marked_at,id)` обслуживает bounded oldest-first batch; повторная отметка использует upsert и не создаёт дубль. Queue очищается только после успешной scoped transaction или активированной full shadow build.
