...
Read-only post-Task-14 re-audit 16.07.2026 подтвердил 73 131 review rows, ноль duplicate non-null ownership/submission keys, duplicate `(review,user)` votes, orphan title targets и неизвестных moderation status. Новая migration, constraint или destructive reconciliation не требуются: публичный профиль использует существующие predicates/indexes через канонический review query.
