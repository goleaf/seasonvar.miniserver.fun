...
- `user_tags` — отдельный private owner aggregate со UUID, original Unicode name, normalized hash, content locale/version, timestamps/soft delete. Текущий create/update flow всегда пишет `content_locale=ru` и не показывает языковой control. `catalog_title_user_tag` связывает запись только с `CatalogTitle` и хранит deterministic `position`; user ID не принимается из request. Season/Episode/Collection/Comment polymorphic tag pivots отсутствуют.
...
- Tag administration загружает и сохраняет только `tag_translations.locale=ru`; новые aliases получают `locale=ru`. Existing non-Russian translations/aliases сохраняются, но RU/EN selector, language heading и подпись «Русский» не входят в authoring UI.
