...
В проектном `.codex/config.toml` включены Laravel Boost MCP и официальный Playwright MCP:
...
Оба сервера не требуют личных токенов. Laravel Boost дает version-aware Laravel-документацию, read-only схему базы, логи и диагностические инструменты. Playwright MCP предназначен для browser QA и запускает управляемый Chromium в headless isolated-режиме без persistent cookies или storage state; временные browser artifacts пишет только в игнорируемый `output/playwright/`. Его failure не блокирует Codex, потому что `required = false`.

В текущей конфигурации `codex mcp list` показывает Laravel Boost и Playwright как enabled project servers. На user-level через `codex mcp` зарегистрированы `openaiDeveloperDocs`, Google Workspace endpoints (`google-gmail`, `google-drive`, `google-calendar`, `google-chat`, `google-people`) и plugin MCP для GitHub, Cloudflare, Figma, Linear, Notion и OpenAI Developers. Google Workspace endpoints пока не авторизованы: Google MCP не поддержал dynamic client registration, поэтому для login нужен заранее созданный OAuth client ID.
...
3. Для Playwright-задач проверить `npx -y @playwright/mcp@latest --version`, наличие совместимого Chromium и после изменения config перезапустить Codex-сессию.
4. Для GitHub-задач проверить connector или `gh auth status`.
5. Для Google-задач проверить наличие OAuth/ADC вне репозитория.
6. Не добавлять активный MCP в `.codex/config.toml`, если он требует личные секреты или не нужен всем разработчикам проекта.
