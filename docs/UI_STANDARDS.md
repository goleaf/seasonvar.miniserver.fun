...
- Центр классификации `/admin/catalog?section=collections` использует text-only адаптивные строки вместо широкой таблицы: checkbox, название, metadata, confidence, причины и category selector сохраняют читаемый порядок на телефоне и desktop. Score никогда не скрывает причину, цвет confidence не является единственным сигналом, а массовое действие всегда разделено на подготовку preview и явное подтверждение. Выбор уверенных ограничен текущей страницей; все controls имеют минимум 44px.
