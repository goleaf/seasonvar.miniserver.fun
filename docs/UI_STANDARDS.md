...
- Длинные группы боковых фильтров `/titles` должны иметь локальный поиск внутри группы как progressive enhancement: без JavaScript все пункты остаются видимыми, с JavaScript пользователь быстро просеивает уже загруженные значения перед отправкой формы.
...
- Поисковая выдача не показывает посторонние карточки при нулевом совпадении. Для поиска, фильтров и полного каталога используются отдельные понятные действия сброса.
