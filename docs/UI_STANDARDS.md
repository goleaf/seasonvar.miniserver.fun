...
- Постер заполняет frame через `object-cover`; небольшой crop по бокам допустим. Внутреннюю ring/border-рамку на изображение не добавлять, если структурная рамка уже есть у карточки.
