...
- Изображение внутри `x-ui.poster-frame` использует `object-cover`, центрирование и `scale-[1.02]`: небольшой crop допустим и нужен, чтобы на дробных размерах не появлялась свободная полоса. У изображения нет собственной ring/border/shadow/padding/rounded-рамки.
- `x-ui.poster-card` задаёт единый структурный каркас `grid`, `horizontal` или `compact`. Только корневой `article` владеет border, внешним скруглением, clipping и тенью; постер примыкает к нему без второго frame.
