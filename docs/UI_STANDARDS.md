...
- Сводные карточки «Найдено / На странице / Сортировка» над выдачей не выводятся. Пагинация плавно возвращает к началу списка, учитывает sticky header и отключает анимацию при `prefers-reduced-motion`.
...
- Боковые фильтры `/titles` автоматически загружаются в deferred Livewire island без кнопки «Показать фильтры». Checkbox/select обновляют связанную выдачу сразу через `wire:model.live`; обычная GET-отправка остаётся только как `<noscript>` fallback. Строки выбора остаются без декоративной обводки вокруг текста.
...
- Блок «Точный подбор» на `/titles` делит расширенные поля на четыре смысловые группы: «Период», «Объём», «Рейтинг» и «Видео». На desktop группы идут в две колонки, на телефоне — в одну; числовые поля имеют компактную `sm:w-*` ширину, но остаются `w-full` на узком экране.
- «Точный подбор» показывает число своих активных условий и раскрывается при их наличии. Действие «Сбросить точный подбор» очищает только период, объём, рейтинг, видео и качества, сохраняя поиск, таксономии, букву, сортировку, вид и размер страницы.
- Мобильная панель выдачи `/titles` даёт доступ к сортировке, виду сетка/список, размеру страницы и алфавиту без дублирования state или JavaScript.
- Мобильная панель выдачи `/titles` даёт доступ к сортировке, виду сетка/список, размеру страницы и алфавиту без дублирования state или JavaScript.
