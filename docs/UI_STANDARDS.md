...
- Служебная статистика `/stats` строится на Livewire 4, обновляется через `wire:poll.1s` и может показывать миниатюры постеров только через внутренний proxy-маршрут, не выводя исходные внешние URL в HTML или Livewire payload.
