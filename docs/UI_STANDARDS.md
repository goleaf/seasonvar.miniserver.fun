...
- На catalog listing routes должна оставаться одна форма с `role="search"`: header search скрывается, а основная форма получает уникальное доступное имя. Активная сортировка объявляется через `aria-current="true"`.
