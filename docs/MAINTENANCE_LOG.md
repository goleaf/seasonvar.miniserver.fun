...
- 13.07.2026: карточка тайтла подключена к уникальному фоновому targeted refresh в `seasonvar-import`: одно успешное обновление действует 15 минут, pipeline разбирает каноническую страницу и все прямые сезоны, а полная Livewire-оболочка обновляется каждые 3 секунды только в видимой вкладке до terminal state. URL и exception details в UI state не сохраняются.
...
- 13.07.2026: постеры всего публичного портала переведены на единый `object-cover` frame без двойной внутренней рамки; удалены сводные карточки над `/titles`, добавлен reduced-motion-aware возврат пагинации к выдаче, плеер расширен, а «Ваш сериал» перенесён под видео. Совпадающий английский суффикс названия теперь выводится отдельной строкой без изменения исходных данных.
