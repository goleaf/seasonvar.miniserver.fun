...
Перед recommendation handoff queued finalizer сохраняет versioned checkpoint уже завершённых bounded maintenance/media/cleanup/merge результатов в summary того же run. Повторная доставка после hard timeout принимает только полный checkpoint текущей версии и не повторяет эти стадии. Recommendation rebuild внутри queue допускает только bounded scoped path; version mismatch, отсутствие active v6 или превышение dirty/source limit дают `deferred`, сохраняют dirty rows и позволяют run стать terminal. Catalog-wide full shadow build выполняется отдельно контролируемым synchronous maintenance-запуском той же команды `seasonvar:import`, а не бесконечными повторами 900-секундного finalizer. После terminal success/failure внутренний checkpoint удаляется. Metadata backlog считается конечным: в него входят только media без детерминированных `format`, `variant_type` или `variant_key`. Неизвестные optional `quality` и `translation_name` честно остаются `null` и сами по себе не заставляют каждый full import повторно обходить те же строки; для реально выбранной строки их безопасное распознавание всё равно выполняется попутно.
