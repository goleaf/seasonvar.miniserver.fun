...
Настройки `seasonvar.media_file_size` (`enabled`, connect/total timeout, retries/sleep, known TTL, unknown/failed retry, chunk и max checks per cycle) читаются из config; environment keys имеют префикс `SEASONVAR_MEDIA_FILE_SIZE_`. Defaults ограничивают нагрузку на provider и не требуют полного backfill при deploy.
