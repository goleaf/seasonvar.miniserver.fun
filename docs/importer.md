...
Импорт не получает второй command или pipeline. After-commit observers синхронизируют только подтверждаемые канонические факты: непустой `Episode::released_at` и реальную publication episode-bound `LicensedMedia`; portal, translation и subtitle events разделены. `release_status_text`, `latest_episode_released_at`, raw provider text и technical timestamps автоматически не превращаются в exact date. Stable logical key делает повтор идемпотентным, manual lock сохраняет правку редактора, а cache/notification side effects запускаются только после committed material change. Полный source hierarchy и ограничения normalization — в [`release-calendar.md`](release-calendar.md).
