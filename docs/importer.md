...
Taxonomy parser сохраняет только каноническое имя, source slug/URL, title, букву, безопасный count и ограниченный список serial URL. Большие описания не импортируются. `SeasonvarTaxonomyIdentity` нормализует HTML entities, Unicode, whitespace, punctuation, case и `ё/е`, а запись справочника проходит через общий `CatalogRelationNameSanitizer`: разные URL и кириллическое/латинское написание одного имени переиспользуют один canonical slug. `CatalogTaxonomyRegistry` остаётся authority model/relation mapping. Provenance хранится через сохранённый первый пригодный taxonomy `source_url` → `SourcePage.url_hash`, crawl/parser timestamps, content hash, missing flags и события. Metadata/RSS snapshots не содержат исходный HTML/XML prose.
...
`CatalogRelationSyncer` — общая граница для будущих importer adapters: он проверяет HTTPS source URL, нормализует имя, объединяет входной пакет по canonical key и выполняет `upsert`/`syncWithoutDetaching`. `SeasonvarCatalogRelationSyncer` перед делегированием дополнительно нормализует URL и разрешает только `seasonvar.ru`. Full cycle и queued finalizer запускают `CatalogMetadataDeduplicator`: сервис через временную SQLite identity-таблицу и bounded chunks переносит pivot-связи, удаляет мусор и дубли во всех десяти справочниках, синхронизирует поисковые документы и остаётся идемпотентным.
