...
Сохранённый `source_type` классифицируется для read-only admin diagnostics как `supported|unsupported|unknown`: фильмовые и cartoon rows не выдаются за дефект сериального matcher, а неизвестный тип остаётся видимой задачей проверки. Классификация не запускает retry/provider HTTP, не создаёт тайтл или membership и не меняет fail-closed year/type matching.
...
Владелец технического retention — production operator runbook Seasonvar. `SeasonvarImportStorageMaintenance` применяет только существующие окна: import events — 7 дней, source snapshots — 14 дней, terminal prepared groups — 7 дней через `SEASONVAR_IMPORT_*_RETENTION_DAYS`; при очистке snapshots всегда сохраняется детерминированная последняя копия. Eligibility требует подтверждённый terminal import run, а для staging дополнительно terminal title group; `queued`, `running`, `discovering` и `finalizing` fail closed. Строки без связанного подтверждённого terminal run автоматически не удаляются.

Один проход имеет общий для events, snapshots, groups и каскадных prepared pages бюджет: chunk `1..1000`, не более `1..100` chunks, `1..100000` фактически удаляемых строк и monotonic time budget `1..600` секунд. Уже начатый SQL batch может завершиться после временной границы, но новый batch не начинается. `preview()` использует те же eligibility queries и возвращает только counts, оценку bytes, самые старые timestamps и counts активных runs/groups; URL, HTML/body, payload, error и credential-like context в DTO отсутствуют.

Независимый `PruneSeasonvarImportStorage` не является второй import command, не выполняет provider HTTP и ставится в существующую очередь `seasonvar-import` без model, URL или payload. Job имеет unique/overlap lock, timeout больше штатного time budget, три ограниченные попытки и безопасный low-cardinality failure log. Daily entry `seasonvar-import-storage-prune` зарегистрирован на `04:17`, но `SEASONVAR_IMPORT_STORAGE_MAINTENANCE_SCHEDULED_ENABLED=false` оставляет dispatch выключенным по умолчанию. Включение запрещено до verified backup/restore, единственного scheduler owner, отсутствия неожиданных writers, безопасного preview и малого canary; удалённые строки восстанавливаются только из backup. Failed jobs автоматически не удаляются, а пользовательская история, legal/admin audit и другие продуктовые данные не входят в importer prune без отдельной policy.
...
## Запись событий импортёра

`SeasonvarImportEventRecorder` является единственным application writer таблицы `seasonvar_import_events`. Он сохраняет warning/error и ключевые lifecycle/terminal события полностью, объединяет высокочастотные успешные события в строки `seasonvar-import-events-aggregated`, детерминированно выбирает часть диагностических success-событий и не сохраняет подробные transient-события. Неизвестное событие уровня `info` по умолчанию остаётся transient; неизвестный warning/error сохраняется.

Aggregate row содержит только отсортированные `counts` по стабильным event codes и общий `total`. По умолчанию flush выполняется каждые `100` событий и на terminal boundary синхронного запуска либо queue apply job; `SEASONVAR_IMPORT_EVENT_AGGREGATE_FLUSH_SIZE` ограничивается диапазоном `1..10000`. Диагностическая выборка по умолчанию сохраняет примерно `1` из `100` стабильных hash buckets через `SEASONVAR_IMPORT_EVENT_SAMPLE_DIVISOR`, также ограниченный `1..10000`. Это эксплуатационная телеметрия, а не полный per-item audit: точные `seasonvar_import_runs` counters остаются источником истины и не зависят от admission policy.

CLI/progress callback продолжает получать каждое событие, даже если durable policy равна `aggregate`, `sampled` или `transient`. Перед записью context проходит существующую очистку URL, credentials, сообщений и путей; ошибка необязательной telemetry-записи не останавливает catalog import. Изменение двух параметров требует обычной пересборки config cache и graceful reload длительно работающих workers. Schema, event codes, queue names и serialized job payloads не меняются.
...
Новый global queued run до начала fan-out сохраняет durable `summary.dispatch_completed=false`. Global finalizer проверяет этот marker до claims/groups и повторно под global lock: строгое `false` запрещает terminal transition даже в коротком промежутке между chunks, а отсутствующий ключ сохраняет compatibility со старыми rows. Только после полного выбора страниц dispatcher под row lock объединяет свежий summary и записывает `dispatch_completed=true`; пустой корректный цикл делает то же до terminal update. Это lifecycle evidence, а не разрешение: live claims, nonterminal groups и global lock остаются обязательными gates.

Подтверждённый преждевременно завершённый run восстанавливается только внутренним `SeasonvarPrematurelyFinalizedRunRecovery`, без нового route/command. Coordinator под тем же start-lock разрешает только exact `sitemap/queue` row со старым `queued_pages`, orphan staging/groups и без другого active global run; live claim без соответствующей prepared row закрывает recovery fail-closed. Service сначала повторно закрывает dispatch barrier, requeue-ит только `queued/preparing` prepared rows, сигналит nonterminal groups, затем атомарно открывает barrier и сигналит обычный global finalizer. Исключение до финального merge оставляет `false`, поэтому повтор безопасен; прямое изменение статуса, массовое освобождение claims, `queue:clear` и `cache:clear` запрещены.

Применение большой title group обязано быть resumable внутри существующего staging contract. После каждого успешно применённого prepared payload финализатор одной транзакцией переводит exact staging row в `applied`, сохраняет bounded media counters в её внутреннем `_application_result`, синхронизирует `group.applied_pages` и добавляет counters в run. Повторная доставка валидирует все terminal rows для общего manifest, но не применяет `applied` row повторно и восстанавливает её counters из checkpoint. Crash между catalog commit и checkpoint может безопасно повторить только одну идемпотентную страницу; crash после checkpoint продолжает со следующей. Queue payload остаётся ID-only, новая schema и отдельная команда не нужны.

Объединение подтверждённого сезонного семейства также ограничено durable season boundary. `SeasonvarTitleMerger` сохраняет прежние canonical identity и доменные merge services, но каждый duplicate season полностью переносит episodes/media/user/community/calendar relations. Все seasons, кроме последнего, удаляются в собственных транзакциях и становятся durable checkpoints; последний season, title-level relations и удаление duplicate title фиксируются вместе, чтобы failure после последнего season не лишил targeted retry общего family hash. Episodes загружаются только для текущего season, а завершённая relation освобождается, поэтому PHP не удерживает весь episode graph семейства до финала. Timeout или exception откатывает только текущую границу, а уже удалённые duplicate seasons не обнаруживаются повторно. Увеличение worker timeout, новая schema, второй merger и прямое исправление данных не используются.

Повторная доставка `PrepareSeasonvarImportTitlePage`, которая находит свою durable строку уже в `prepared|applied`, сначала освобождает только принадлежащий той же странице и run точный claim token, затем сигнализирует group finalizer. Перед reconcile group finalizer тем же условным `run + token` контрактом освобождает остаточные claims только у terminal prepared rows; не-terminal работа и чужой/сменившийся token не затрагиваются. Поэтому crash между фиксацией terminal state и прежним release больше не оставляет orphaned claim, который ошибочно удерживал global single-flight и выдавал оператору «процесс уже запущен». Очередь, claims и failed jobs ради восстановления не очищаются и не переписываются.

Перед recommendation handoff queued finalizer сохраняет versioned checkpoint уже завершённых bounded maintenance/media/cleanup/merge результатов в summary того же run. Повторная доставка после hard timeout принимает только полный checkpoint текущей версии и не повторяет эти стадии. Recommendation rebuild внутри queue допускает только bounded scoped path; version mismatch, отсутствие active v6 или превышение dirty/source limit дают `deferred`, сохраняют dirty rows и позволяют run стать terminal. После terminal завершения ставится уникальный `RebuildCatalogRecommendations` на очередь `seasonvar-import`, а catalog-wide full shadow build выполняется worker отдельно от finalizer. После terminal success/failure внутренний checkpoint удаляется. Metadata backlog считается конечным: в него входят только media без детерминированных `format`, `variant_type` или `variant_key`. Неизвестные optional `quality` и `translation_name` честно остаются `null` и сами по себе не заставляют каждый full import повторно обходить те же строки; для реально выбранной строки их безопасное распознавание всё равно выполняется попутно.

`ExternalMediaMetadata` дополнительно вычисляет nullable
`subtitle_language` только рядом с явным маркером `subtitles/subs/субтитры`
и allowlisted language marker. `ExternalPlaylistImporter`,
`SeasonvarCatalogImporter` и уже выбранные metadata-backlog rows записывают
это поле вместе с `variant_*` и `has_subtitles`; язык интерфейса, страна и
произвольная часть URL не считаются доказательством. Неизвестный язык
остаётся `null`, не создаёт бесконечный backlog и не требует скачивания
video body.

Уникальный `WakeSeasonvarImportFinalizers` каждые десять минут страхует потерянный transport и terminal signal. Перед прежним `wakeReady()` он передаёт текущий active global run в `SeasonvarActiveRunReconciler`. Reconciler использует `seasonvar_import_prepared_pages` как durable ledger, после полного stale-window переводит потерянный `dispatch_completed=false` в подтверждённый recovery-barrier и по compare-and-swap повторно ставит не более `SEASONVAR_QUEUE_FINALIZER_WATCHDOG_BATCH_SIZE` старых `queued|preparing` строк. Indexed query читает ещё одну неизменяемую sentinel-row: она только доказывает наличие следующего пакета и устраняет повторный общий `exists()` по журналу. Уже projected `SeasonvarImportTitleGroup` переиспользуется для deduplicated finalizer signal без отдельного `find()` на каждую группу. Для старого non-serial claim без staging row повторно используется существующий ID/token payload. Следующая ID-only reconciliation job находится в той же очереди, поэтому восстановление создаёт bounded backlog с естественным backpressure, а не отправляет десятки тысяч сообщений одной транзакцией. Повторная доставка остаётся идемпотентной по durable status и existing unique job keys; claim не освобождается и повторно проверяется worker.

Обычный `php artisan seasonvar:import` выполняет тот же bounded reconcile перед отказом создать второй sync run. Поэтому оператор видит, сколько задач возвращено в очередь, а уже активный global run продолжает работу вместо вечного сообщения без прогресса. `--status`, URL, inventory и size-only не запускают recovery. Реальный batch re-dispatch обновляет heartbeat; сам finalizer при закрытом `dispatch_completed` больше не изображает прогресс. Watchdog не выполняет provider HTTP и catalog apply внутри собственного `handle()`, не очищает очередь/cache/failed jobs и не делает незавершённую группу terminal. Ошибка постановки signal или page job не откатывает уже durable состояние: в журнал попадают только IDs и класс ошибки, а следующий due-ledger scan безопасно повторяет попытку.

Global stale recovery больше не закрывает `running` run только из-за истёкшего heartbeat/claim, пока существуют `queued|preparing|prepared` staging rows либо `discovering|running|finalizing` title groups. Такие строки являются воспроизводимой durable work и принадлежат reconciler/finalizer watchdog. Run без live claim, staging и active group сохраняет прежний автоматический terminal failure, поэтому действительно пустая зависшая запись не блокирует новый импорт.

Stale reconciliation выполняет сам finalizer в короткой transaction и повторно проверяет active run, timestamp и отсутствие live claim незавершённой строки. `queued/preparing` rows считаются необратимо потерянными только после `max(retry_window, claim_window) + 300s`; они получают `preparation_deadline_exceeded`, а подготовленные siblings всё равно применяются с итогом `partial`. Нулевой/неполный page set, отсутствие любой применимой страницы и исчерпание finalizer deadline завершаются `failed` с allowlisted codes `empty_page_set`, `page_set_mismatch`, `no_prepared_pages` или `finalizer_deadline_exceeded`. Код хранится отдельно от user-safe русского сообщения; raw exception, URL, HTML и queue payload туда не попадают. Fresh nonterminal work и claims других runs не изменяются; отдельное exact-освобождение claim terminal staging-row является нормальным завершением lease, а не stale state rewrite.
...
The primary review migration uses provider/published defaults, and idempotent repair `235100` only converges four missing lifecycle columns plus nullable report dedup, so existing importer SQL and the historical 73 101-row baseline remain compatible before/after rollout. The 18.07.2026 census measured 73 184 provider rows: 73 105 published and 79 retained merge tombstones, with no provider row linked to a user. Removed-state repair `2026_07_18_220000` touches only missing deletion evidence on already `removed` rows and preserves provider ID/body/hash/source/title; existing merged evidence wins over the moderator fallback. `ReviewSchema` keeps the legacy API/read path active during partial deploy and now withholds community writes until every lifecycle column is present. Provider bodies may exceed community limits and are preserved byte-semantically; community value-object validation is not retroactively applied to imported content.
...
Каждое новое глобальное назначение вне migration backfill обязано получить current provider/editorial provenance в той же domain boundary. Demo data больше не пишет `catalog_title_tag` и использует только личные owner-scoped теги. Исторический `seasonvar-demo-v1` footprint исправляется отдельно существующей production-gated командой: восстанавливается exact deterministic selection, удаляется только пара без любого current provenance, собственные demo-теги подтверждаются одновременно versioned `code` и deterministic UUIDv5 `public_id` и архивируются без hard delete. Это не расширяет обычную stale convergence импортёра до опасной очистки всех legacy pivots.
...
`php artisan seasonvar:import` остаётся единственной public import command. Full sync передаёт catalog-wide v6 shadow rebuild в уникальный `RebuildCatalogRecommendations` на очереди `seasonvar-import`, а queued finalization делает тот же handoff после bounded scoped stage. `CatalogTitleRecommendationBuilder` atomically replaces only calculated rows and сохраняет прежние active rows до quality-gated activation. Completed worker rebuild bumps existing recommendation cache version once. No HTTP inference, external recommender, schema change or second command was added.
...
`SeasonvarCatalogPagePreparer` до записи каталога получает bounded availability metadata и сохраняет её в prepared payload. `SeasonvarCatalogImporter::applyPreparedPage()` только применяет этот результат: provider availability и file-size HTTP из него недостижимы. Новый media и запись с изменившимся effective `playback_url`/`path` получают `pending`; title-group apply-lock покрывает каталожные записи, но не provider latency.

`InspectLicensedMediaFileSize` остаётся единой action для `ExternalPlaylistImporter` и bounded file-size backlog. Перед запросом action фиксирует title/effective URL/format и сохраняет ответ только conditional update с теми же значениями; concurrent source change оставляет новый `pending`, выдаёт безопасный skipped event и не получает размер старого URL. Неизменённый `known`, `unknown` или `failed` соблюдает свой TTL/retry, `unsupported` без force не проверяется повторно. Ошибка размера не увеличивает page/media failure и не делает playable media недоступным.
...
## Playback synchronization Task 07

`seasonvar:import` остаётся единственной public command. Source import сохраняет stable episode relation, provider/source identity, `source_media_key`, original provider metadata, normalized variant/type/quality/format/subtitle flag, audience/window, priority/status/health и editorial corrections по существующим ownership rules. Duplicate detection не объединяет строки по похожей visible подписи; replacement источника не переносит progress, потому что progress связан с `Episode.id`.

Importer не создаёт audio/subtitle/region/age данные из догадок, не сохраняет short-lived grants и не скачивает video body. Add/remove/disable/priority/health/profile mutations продолжают targeted cache invalidation и обновляют player options после commit. Полный playback/import compatibility contract: [`audits/video-playback-report.md`](audits/video-playback-report.md).

## Personal-library compatibility Task 09
