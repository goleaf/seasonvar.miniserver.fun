...
Настройки `seasonvar.media_file_size` (`enabled`, connect/total timeout, retries/sleep, known TTL, unknown/failed retry, chunk, max checks per cycle, `scheduled_backfill_enabled` и `scheduled_backfill_limit`) читаются из config; environment keys имеют префикс `SEASONVAR_MEDIA_FILE_SIZE_`. Defaults ограничивают scheduled пачку 20 проверками, а pipeline дополнительно clamps любой CLI/config limit; полный backfill при deploy не требуется.
