...
`licensed_media` хранит exact nullable `file_size_bytes` (`null` означает unknown, доверенный `0` — zero-byte), `file_size_checked_at`, enum-compatible `file_size_check_status`, bounded `file_size_source`, `file_size_http_status` и sanitized `file_size_check_error`. Форматированная строка в БД не хранится. Terminal events: `seasonvar-media-size-known|unknown|unsupported|check-failed`; started/skipped и backlog started/complete дают подробный русский progress без raw URL. `seasonvar_import_runs` атомарно агрегирует checked/known/unknown/unsupported/failed и сумму найденных exact bytes одинаково для sync и queued finalization.
...
php artisan seasonvar:import --refresh-media-sizes --media-size-limit=100
php artisan seasonvar:import --refresh-media-sizes --force-media-sizes --media-size-limit=100
```

Backfill идёт stable ID order через `lazyById`, eager-loads только title/season/episode context, соблюдает stop signal и сохраняет каждый готовый результат. Режим несовместим с URL, `--queued`, `--forever`, inventory/status и page-type selection; обычный full/queued import всё равно автоматически проверяет новые/изменённые media и выполняет консервативный bounded due backlog.

Laravel scheduler каждые 10 минут запускает тот же size-only mode с `--media-size-limit`, `withoutOverlapping(30)` и `onOneServer` через общий `redis-locks`. Команда дополнительно наследует глобальный importer lock: активный sync/queued import не получает параллельный inspector. Schedule не работает в maintenance mode, не запускается background process и не делает global catalog cache bump; реально изменённые размеры уже инвалидируют только affected title.

Настройки `seasonvar.media_file_size` (`enabled`, connect/total timeout, retries/sleep, known TTL, unknown/failed retry, chunk, max checks per cycle, `scheduled_backfill_enabled`, `scheduled_backfill_limit` и `status_cache_seconds`) читаются из config; environment keys для изменяемых deployment-параметров имеют префикс `SEASONVAR_MEDIA_FILE_SIZE_`. Defaults ограничивают scheduled пачку 20 проверками, а pipeline дополнительно clamps любой CLI/config limit; полный backfill при deploy не требуется.

`LicensedMediaFileSizeBacklog` является общей eligibility/freshness boundary для pipeline, `seasonvar:import --status` и административного снимка. CLI status дополнительно показывает global eligible/checked/pending/due/terminal counters, coverage, exact/human known bytes и время capture. Admin page показывает те же безопасные агрегаты без URL и не инициирует HEAD/Range при render. Один aggregate scan кешируется в existing operational `TieredCache` на `status_cache_seconds` (default 900 секунд) с bounded stale fallback; отдельные media writes намеренно не инвалидируют снимок, поэтому видимые totals имеют не более пятнадцати минут eventual lag и не создают scan storm во время backfill.
