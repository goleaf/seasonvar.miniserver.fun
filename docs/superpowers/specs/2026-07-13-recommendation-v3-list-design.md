...
The media stream reuses the existing `(status, published_at, id)` feed index. Episode events receive one additive SQLite-compatible index matching publication/deletion/release ordering; no summary table, trigger or backfill is introduced. On current data, the first 10,000 media events contain 252 unique titles and the bounded prototype returned all 180 eligible candidates in 1.697 s with 34 MiB peak memory. A 20,000-event diagnostic completed in 1.911 s with 46 MiB peak memory. Exact post-change figures are recorded only after implementation verification.
