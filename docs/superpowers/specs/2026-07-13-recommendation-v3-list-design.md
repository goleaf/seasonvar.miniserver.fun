...
### Post-rollout pagination-prefix stability

The pagination acceptance audit found a latent ordering dependency in the diversity stage. Candidate generation is deterministic, but `CatalogRecommendationService::result()` previously asked the diversity service to evaluate only the prefix needed for the current page. `CatalogRecommendationDiversityService` derives each title's dominant genre and actor from feature frequency inside the supplied candidate set and derives franchise components from relations inside that same set. Expanding the set from page one to page two can therefore change those derived assignments and retroactively reorder an already displayed prefix, even when the current production snapshot does not happen to trigger the condition. That violates the stable-page contract and can create skipped or repeated cards across navigation as catalogue metadata evolves.

Three approaches were evaluated. Persisting a versioned diversified order would add user/cache state and invalidation ownership for behavior that is already fully deterministic. Choosing a permanent per-title dominant genre/actor would require a new editorial or aggregate truth and would weaken cohort-aware diversity. The selected design diversifies the complete already-bounded candidate pool once per result, then slices the requested page plus its look-ahead row from that canonical order. Public and personalized candidate builders already cap the pool at the configured 180 rows with a hard maximum of 500, so the change remains bounded and reuses the existing two pivot queries plus optional relation query without scanning the catalogue.

Page, `perPage`, filters, type, locale, seed, exclusions and cache candidate identity remain unchanged. Diversity still caps the same dominant genre, actor and explicit franchise groups and still fills from deferred rows for a small catalogue. The only semantic correction is that every page is now a slice of one full-pool order; later pages cannot rewrite earlier ranks. No migration, new cache key, persistence, route, DTO, translation, frontend asset or background owner is introduced.
