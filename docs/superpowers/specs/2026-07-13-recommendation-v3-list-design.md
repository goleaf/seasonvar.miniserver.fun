...
| `recently_added` | `catalog_titles.created_at`, стабильный marker создания нового опубликованного импортом тайтла, после canonical visibility | публичный | public cache; indexable default state |
...
В схеме нет отдельного title-level `published_at`: основной importer создаёт новый тайтл уже опубликованным в той же транзакции, поэтому `created_at` точнее технического `indexed_at`, изменяемого переиндексацией. Поздняя публикация долго существовавшего draft остаётся известным ограничением и не подменяется искусственным timestamp/backfill.
...
Candidate generation читает не более 120 recent meaningful source titles. Значимым считается completion, минимум 180 секунд или 10% прогресса; page open/нулевая позиция не является интересом. На source title накладывается один strongest verified signal: explicit `more_like_this`, completion/history, watchlist, `planned|watching`, high portal rating, own collection membership или own personal-tag assignment. Все bounded ID collections явно накапливаются в одном server-side signal map; `not_interested`, `blacklisted` и `dropped` удаляются из source map до чтения candidates, поэтому отрицательный intent не может породить «похожие» результаты. Затем читаются только первые 24 stored similar rows на source; полный каталог в PHP не сканируется.

Hard exclusions централизованы: current title, explicit page exclusions, любой title с обработанным recommendation feedback, recently shown IDs при refresh, недоступные записи и source titles. General personalized discovery также исключает/demotes watching, completed и dropped. `more_like_this`, `planned` и watchlist служат положительными source signals, но сам сохранённый title не выдаётся как новый. Completed/dropped не удаляются из каталога и остаются доступны по прямой ссылке, в explicit similar/related и upcoming/new-release context. Upcoming использует watched/completed title только как boost, поэтому новая серия может вернуть релевантный завершённый сериал без бесконечного повтора в «Для вас».
...
Recently displayed IDs are stored only in the current server session, separately for guest and authenticated user, at most 96 IDs for seven days. Refresh remembers the visible batch, changes an opaque seed and demotes/excludes immediate repeats; authenticated homepage personalization similarly remembers only its rendered eight IDs. Anonymous homepage remains unseeded and shared-cacheable. Personalized cold-start public fallback keeps watching/completed/dropped exclusions; if the eligible catalogue is too small, it relaxes only repeat suppression. It never permanently blocks a title and is removed with session/account lifecycle. No impression analytics table or write per card was added.
...
The public trending watchlist branch is present only when `watchlist_updated_at` exists and reads that column; a rolling deployment without the new column simply omits that contribution instead of falling back to technical `updated_at`. Personalized signal windows use the matching semantic column and fall back to stable row identity when the additive schema is not yet available. Migration `230000` replaces the two Task 18 indexes that encoded technical `updated_at` with exact semantic activity indexes and restores the legacy index definitions on rollback. A separate reversible `230100` migration adds owner-first covering order indexes for only the three bounded personalized watchlist/status/rating windows: disposable SQLite plans otherwise required temporary order B-trees, while these exact indexes satisfy filtering, semantic ordering, stable ID tie-breaking and title-ID projection without a table lookup.

Title merge also reconciles the complete one-row state rather than only watchlist/rating: `blacklisted` wins over `not_interested`, `not_interested` wins over `more_like_this`, and `dropped` wins over `completed`, `watching` and `planned`. Versions remain monotonic. This conservative precedence prevents a duplicate-title merge from silently re-exposing content the user explicitly hid or dropped.
...
Homepage renders one light recommendation row: authenticated real personal result or honest public fallback, anonymous trending/popular. Title detail orders related then similar and offers working feedback/undo. Recommendation cards explicitly label truthful broad reasons and authenticated discovery/title-detail controls explain the consequences of «Больше похожего», «Не интересует» and «В чёрный список» before mutation. Search no-result links to explicitly labelled popular discovery rather than pretending those cards matched the query. Library exposes personal discovery and negative-feedback restore. Future episode/title dates power upcoming discovery; there is no competing calendar model. Existing cards, light theme, Russian/English translation architecture and local assets are reused.
...
### Post-rollout pagination-prefix stability audit

The pagination acceptance audit evaluated whether increasing the requested page could change dominant genre/actor or explicit-franchise assignments and retroactively reorder earlier cards. The initial concern was rejected after tracing the actual call boundary: `CatalogRecommendationService::result()` always passes the complete already-bounded candidate array into `CatalogRecommendationDiversityService`. The second argument controls only how many selected rows the service returns; feature frequency and relation components are calculated from the same full pool for every page. The selection loop is prefix-deterministic, and deferred fill appends the same ordered deferred prefix when a small catalogue cannot satisfy a limit.

A controlled diagnostic can make diversity change only by replacing the candidate array itself. That represents a new ranking input/generation, not pagination within one result version. Stable public contexts reuse the same cached scalar pool; seeded/private contexts rebuild from deterministic inputs for their server-held seed and exclusions. Catalogue mutations select a new recommendation cache generation by design and are allowed to refresh the ranking.

Read-only verification compared page 1 plus page 2 with a single 48-row window for trending, popular, Kinopoisk top-rated, recently added and recently updated. Every pair had zero overlap, the concatenation matched the canonical 48-row prefix, page-two ranks began at 25 and `hasMore` remained correct. No PHP, migration, cache key, persistence, route, DTO, translation, frontend asset or background change was justified.

### Post-rollout editorial cache invalidation scope

The invalidation audit found that `CatalogCollectionCacheInvalidator` bumped the public Recommendations generation for every collection mutation, including private user collections, unlisted collections, unapproved drafts and unfeatured editorial collections. Those records are legitimate private personalization signals, but personalized recommendations are intentionally uncached. Public editorial discovery reads only non-deleted, published, approved, public, featured collections of the stable `editorial` type. A private membership edit therefore could not change any shared recommendation payload, yet it invalidated every public popular/trending/top-rated/new/editorial candidate snapshot.

Keeping the global bump is safe for freshness but creates unrelated public cold misses and conflicts with the targeted-invalidation contract. Adding a second editorial invalidator to each collection write service would duplicate lifecycle logic and risk missing transitions away from eligibility. The selected design keeps the existing collection invalidator as the single mutation boundary and makes only its Recommendations bump conditional. It evaluates the collection's current raw attributes and Laravel's previous saved attributes, so both entry into and exit from public editorial eligibility invalidate. Unknown/bulk calls remain conservative. Title-driven collection invalidation narrows its lookup to exactly eligible editorial collections before requesting a recommendation bump.

Collections, Homepage, Sitemap, API and TitleDetail generations retain their existing collection-domain behavior; this follow-up changes only whether the Recommendations domain is also bumped. No private collection name, item, owner or ID enters a shared key or payload. Account-wide calls without sufficient retained model context still invalidate conservatively. The change introduces no schema, route, cache key/version, new cache service, personalization cache, translation, frontend asset or background dependency.

### Post-rollout comment activity invalidation scope

The next cache-lifecycle trace found that every changed title comment called the public recommendation invalidator. This included reaction changes, body/spoiler edits that stayed published, pending comment creation, author-name changes and account anonymization. None changes the ranking input: trending counts only published, non-deleted comments whose stable target type is `title` and whose creation time is inside the selected period. Season, episode and collection discussion are not recommendation activity. The previous behavior was fresh but allowed unrelated high-frequency engagement to evict all shared public candidate snapshots.

The selected design retains `CommentCacheInvalidator` as the only comment/cache boundary and preserves all existing title/collection presentation invalidation. Existing public method signatures gain only an optional boolean with a conservative default, so merge/unknown callers remain backward compatible. Action owners compute whether published visibility actually changed; the invalidator additionally requires a title target before bumping Recommendations. Create passes true only for a published row, update/delete/restore/moderation compare pre/post published-and-not-deleted state, and reaction passes false. Author identity and account anonymization preserve public comment counts and therefore pass false. Title merge retains conservative recommendation invalidation because it can remap counted activity.

No comment body, reaction identity, user identity, moderation note or target ID enters a cache key, payload or telemetry value. The change does not alter comment authorization, moderation, notifications, counts, HTML cache scopes, trend weights, period bounds, recommendation identity, routes, schema, translations, frontend assets or background infrastructure. It narrows only an invalidation trigger and therefore requires no cache flush or stored-data rewrite.

### Authorization, privacy, analytics and lifecycle

Public discovery is readable without login. Feedback/watch status uses current authenticated user and `CatalogTitlePolicy::interact`; editorial relation create/remove/search requires `manage-catalog`. Client-provided IDs, type, relation type, priority, feedback, filter, provider and limit are enum/allowlist/bounds checked again in services. The query-free feedback component additionally allowlists the two internal Livewire action names, so dynamic directive text cannot be injected. Editorial text is existing escaped collection metadata; no raw HTML or score mutation reaches mass assignment.

No recommendation impression/click/conversion analytics existed, so none was invented. Trending uses already persisted semantic portal activity only. Account export includes stable `more_like_this|not_interested|blacklisted|null` feedback plus watch status; account deletion cascades user-title state, progress/session repeat suppression and private tags/collections through existing lifecycle, while public calculated/editorial relations remain. Public HTML/structured data never contains another user's activity, private explanations or access state.
