...
- Viewport-lazy `@island(name: 'catalog-live', lazy: true)` и три одноимённых result islands связаны Livewire 4.3.3. `wire:model.live` checkbox возвращает realtime-обновление результатов, счётчиков и URL-state, а повторный `wire:intersect.once` после route navigation не оставляет фасеты в placeholder.
