...
Статус: implementation, production index rollout, rolling runtime activation, task-focused/full-suite verification и Git delivery завершены; post-delivery fix обхода `ShouldBeUniqueUntilProcessing` опубликован в `096c66f`, а read-only census подтвердил отсутствие новых общих дублей. Выявленное финальным health ограничение `TD-012` закрыто отдельным bounded heartbeat follow-up: lease точной cache-warm connection/queue пары теперь покрывает разрешённую 600-секундную `WarmCatalogCaches` плюс 60 секунд запаса.
...
| Production rollout | completed | Import workers естественно переработались в 02:58 без ручного restart. Два read-only census за 91 секунду при продвижении active run на 42 страницы показали недельный unique lock и неизменные 614 legacy общих jobs; новые общие дубли не появились, legacy backlog оставлен для естественного drain без queue clear/rewrite. Последующий `TD-012` follow-up выполнил graceful recycle после активной job: cache-warm heartbeat сохранился за прежней 120-секундной границей, следующая job получила TTL 643 секунды при lease 660, а health показывал реальный `degraded` backlog вместо ложного `failed`. Queue clear/rewrite/retry не выполнялись. |
