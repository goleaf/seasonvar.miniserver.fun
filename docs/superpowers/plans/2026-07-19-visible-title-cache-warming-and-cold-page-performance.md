...
| Production rollout | unresolved | Backup, target-only index и основной title runtime rollout завершены. Import workers естественно переработались в 02:58 и подхватили fix без ручного restart; Redis показывает новый недельный unique lock. Нужны повторный census и terminal-run drain без queue clear/rewrite. |
