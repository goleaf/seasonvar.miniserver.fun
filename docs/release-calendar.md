...
выводит уже упорядоченную page collection с сохранением её порядка.

### Пакетные публикации серий

HTML-календарь объединяет синхронные episode-bound события одного тайтла в
одну карточку только при совпадении тайтла, сезона, типа, статуса, точности,
точного `starts_at`, признака оценки, перевода и языка. Событие без
`episode_id`, числового `episode_number` или точного времени всегда остаётся
отдельным. Разные сезоны, времена, типы, статусы и переводы не склеиваются.

Пагинация выполняется по semantic group keys, а не по исходным строкам:
пачка больше `release-calendar.per_page` не разрывается между страницами.
Второй bounded query загружает членов только для групп текущей страницы,
минимальные проекции `catalogTitle`/`episode` и одну карту подписок.
Повторные медиаварианты одной серии выводятся как одна серия; пропуски в
номерах форматируются точными диапазонами (`185–186, 188, 190–192`), а не
ложным непрерывным интервалом.

Карточка показывает краткую type-aware фразу и нативный
`<details>/<summary>` со списком серий и их названиями. Одиночная карточка
сохраняет прежний вид. Канонические строки, счётчики месяца, уведомления,
private iCalendar, corrections, admin, import, API и cache invalidation не
группируются и не меняют identity.
