...
Collection directory/dashboard/editor/page/profile/title selector/admin queue reuse the existing light panels, form/status components, list title cards and pagination. Names/descriptions wrap without truncating user content, forms collapse to one column, controls retain 44px targets, and destructive actions are separated/confirmed. After Task 56, collection images are absent from every current surface: navigation, directory, cards, details, profiles, editor and administration remain text-only; a separate compatibility cleanup owns legacy cover values. The manual editor adds a real Livewire drag handle within one page; up/down remains the keyboard/touch/no-drag ordering baseline, and no hover-only control exists.
