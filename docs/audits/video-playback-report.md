...
После принятого seamless transition серверный commit повторно проверяет пару
episode/media и перерисовывает только именованный Livewire island
`catalog-player-navigation`. Его bounded projection содержит текущую серию,
двух соседей и canonical fallback URLs без media/source/grant URL. Поэтому
видимые «Предыдущая/Следующая» сразу соответствуют новой серии, а
`wire:ignore` media shell, `<video>`, Plyr и fullscreen root не участвуют в
morph. Ordinary `href` остаётся безопасным full-page fallback при отказе
JavaScript или island transport.

Adjacent anchors намеренно не содержат `wire:click`: действие
`selectEpisode()` внутри navigation island меняло бы URL и соседние ссылки, но
не могло бы заменить `wire:ignore` video shell. После инициализации
`CatalogPlayerSession` capture-handler выполняет server-authorized seamless
transition и вызывает
`$wire.$island('catalog-player-navigation').commitPlayerTransition(...)`.
Если runtime ещё загружается, уже завершил `pagehide` cleanup или недоступен,
реальный `href` выполняет полный внутренний `GET`, и новый HTML согласованно
выбирает episode, media, video и navigation.
...
- Global playback fallback принадлежит существующему `CatalogPlayerSession`, использует его единственный document listener и уничтожается тем же `AbortController`. Проверки external editable/interactive controls и `dialog[open]` выполняются до глобального `Shift+E`, поэтому заглавная `E` остаётся обычным вводом, а player menu не открывается поверх существующего dialog; player-owned opener и controls сохраняют menu shortcut. Системные `Alt`/`Ctrl`/`Meta` combinations и отключённая keyboard preference также не перехватываются; pointer/touch controls остаются полным альтернативным способом действия.
- Основные touch actions продублированы одним центральным рядом внутри текущего Plyr container: `−10 / play·pause / +10`. Side targets ограничены `56..80 px`, toggle — `68..96 px`; три native buttons центрированы по обеим осям, используют существующие RU/EN Plyr labels и тот же `seekMediaBy()`/`togglePlay()` path, останавливают всплытие click до Plyr `clickToPlay` и синхронизируют состояние по native `play`/`pause`/`ended`. Они создаются и уничтожаются текущей `CatalogPlayerSession`; стандартный `play-large` скрывается только под ready-marker после успешной initialization.
...
Task 20 от 24.07.2026 добавил regression для порядка global keyboard exclusions. RED воспроизвёл потерю ввода в `#site-search` как `Expected "E", Received ""`; минимальный GREEN применил уже существующие external-interactive/open-dialog проверки до `preventDefault()` и открытия player menu, сохранив shortcut на player-owned opener. После свежей Vite-сборки focused Desktop Chromium прошёл `2/2`, focused player PHP — `114` тестов и `1492` утверждения, а полный `player-lifecycle.spec.js` — `15` сценариев при `12` ожидаемых mobile/tablet skips. Native iOS fullscreen остаётся `unresolved_device`.
