...
| SEC-08 | Confirmed control | No `@php`, DB/Auth/Gate/Storage facade calls or PHP blocks in Blade | Extend test to forbid `request`, `config`, application static/service logic | Partial: strict calls still present | 41 `request()` + 1 `config()` violate passive Blade architecture |
