...
| PERF-08 | Confirmed control, repository audit complete | Literal eager loads across `app/` use selected relation columns and required keys; runtime SQL covers public tags/collection export/admin detail | Preserve AST/runtime regressions; profile production memory separately | Implemented | Only `SeasonvarTitleMerger` intentionally loads a complete mutable relation aggregate; dynamic query maps remain owner-tested |
...
## Eager-load projection audit

The repository-wide AST inventory covered literal `with()`, `load()` and `loadMissing()` calls rather than sampling only catalog controllers. Public/API/list/paginated/export reads now select related primary keys, required parent/child keys and presenter-owned fields. The same rule was applied to large importer reads: playlist candidates, title manifests, refresh planning, source-page preparation, stale-group reconciliation and group finalization no longer fetch unrelated relation columns. Focused finalizer tests exposed and then locked the less obvious partial-model requirements (`provider_field_values`, source identity and run/group/claim keys).

`SeasonvarTitleMerger` remains the sole explicit exception because it mutates and transfers the full alias/rating/taxonomy/season/episode aggregate. This is not a measured production latency claim: runtime SQL tests prove the selected relation shapes, while memory and p95 changes still require a controlled production workload under the measurement protocol above.
...
Indexes match these exact predicates and are documented in `DATA_RELATIONS.md`; denormalized counts/score were rejected to avoid drift. Private block/mute lists now have independent paginators and exact `(blocker_id,id)` / `(muter_id,id)` covering indexes, while moderator presentation skips both personal relationship reads. Disposable SQLite `EXPLAIN` selected those indexes and `comments_target_list_idx`; the browser confirmed progressive reply loading rather than a full initial thread. Shared guest HTML may contain only public DTO, while auth/Livewire overlays bypass it; mutations bump only affected existing title/collection versions. Production cardinality/latency remains a rollout measurement and no fabricated improvement percentage is reported.
