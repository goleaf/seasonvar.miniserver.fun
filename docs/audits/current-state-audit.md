...
Реализация добавляет одну collection aggregate с UUID identity, user/editorial/system types, private/unlisted/public semantics, moderation/reporting/feature, slug history, двухуровневой управляемой категорией, manual item uniqueness/order, owner-only dynamic smart rules, public/owner/admin/API/search/recommendation/profile/account/merge/cache/SEO/sitemap integrations и locale-aware editorial/category translations. Первоначальные `/collections`, `/lists`, `/selections` и `/admin/collections` позднее удалены без redirects по продуктовому решению: public explorer теперь вложен только в `/discover/popular`, а управление коллекциями — только в `/admin/catalog`; detail/profile/API contracts сохранены. Собственные изображения, cover route, импорт и storage metadata подборок удалены. Smart mode хранит versioned allowlisted criteria и вычисляет private owner result при чтении без materialized membership; collection likes/follows/collaborators остаются explicit unsupported boundaries, а не fake controls.
