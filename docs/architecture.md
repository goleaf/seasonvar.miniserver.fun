...
- `/titles/{catalogTitle:slug}` сохраняет `CatalogShowRequest`, implicit route binding, historical-slug redirect и SSR SEO в тонком controller/view слое. `App\Livewire\CatalogTitleDetail` владеет полной динамической оболочкой страницы, а вложенный `CatalogTitlePlayer` отдельно отвечает за URL-state активного сезона/серии/media и authenticated user actions. Оба компонента держат `catalogTitleId` locked; Eloquent-коллекции существуют только как render data.
