...
- Mobile API добавляется рядом с legacy API через `/api` discovery и версионированную группу `/api/v1`; небольшие invokable controllers отвечают за discovery/config/health/OpenAPI, а `AssignApiRequestId` и централизованный `ApiErrorResponse` задают общий transport contract. Legacy controllers/resources не заменяются v1-классами.
