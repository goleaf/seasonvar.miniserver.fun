...
- На первом проходе `CatalogTitleQuery` запрос только из стоп-слов дает нулевое условие без `title` context, но сохраняет существующий `whereKey()` для title-scoped страниц. Пока временный no-year fallback не удален, `CatalogTitlesPageBuilder` может заменить этот unscoped-ноль опубликованным каталогом в итоговом HTTP-ответе.
