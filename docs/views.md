...
- `x-ui.poster-card` собирает poster frame и body в строгих layout `grid`, `list`, `compact`, `recommendation`, `stats`, `home`, `spotlight`, `trend` и принимает optional named `mediaOverlay`; он не создаёт wrapping anchor. Обычные карточки используют `border-slate-200` без тени, а поднятый action menu — отдельный `shadow-elevated`.
- `x-catalog.title-card` даёт `CatalogTitle` один основной title tab-stop и отдельные постоянно доступные grid/list actions. Неизвестный layout нормализуется в `list`; `compact` имеет собственный совместимый template. Class component не вызывает lazy loading и читает только агрегаты, подготовленные `card_*` attributes или уже загруженные relations. Grid не получает description, показывает максимум два жанра и один rating; list получает excerpt максимум в три строки, country, counts и оба rating.
- `components.catalog.title-card-actions` выводит обычную watch/resume ссылку, policy-backed library button, details и нативный `details` menu для существующих feedback reasons. Все controls имеют видимый focus и минимум 44 px; coarse pointer получает fixed safe-area sheet, fine desktop pointer — absolute dropdown. Status/error находятся в `catalog-pagination` island и обновляются тем же Livewire action.
...
`CatalogDiscoveryPage` является единственным full-page owner публичного discovery. Девять режимов имеют один H1 и реальную переносящуюся навигацию; refresh является secondary results action, а общий filters `<details>` закрыт без условий и открыт при URL state. Каждый mode выводит query-free `#collections` и собственный title-results anchor, затем монтирует единственный `CatalogCollectionExplorer` перед serial results; `popular` сохраняет `#popular-titles`, остальные modes используют `#discovery-titles`. Explorer получает стабильный key mode+locale, хранит URL-backed search/sort/root/subcategory/page, получает готовый полный active root/child navigation array с scalar counts и `is_filterable`, рендерит один nested list и compact text-only collection cards. Blade не фильтрует counts и не выполняет запросы: zero-count nodes отображаются как prepared noninteractive labels. `CatalogAdministrationPage` аналогично владеет `/admin/catalog`; вложенные компоненты не расширяют layout и не создают второй `<h1>`.
...
Компонент `x-content-requests.correction-link` удалён: title/player views не получают correction URL и не выводят field/episode/subtitle controls. Закрытая admin form показывает server-resolved read-only context и пять radio-причин только для тега. Card/detail/admin presenters выводят translated field/reason и safe current/proposed values без запросов и private evidence; public presenter не выдаёт engagement для administrative-only request.
