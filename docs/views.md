...
- Класс-компонент `App\View\Components\Catalog\TitleFilters` получает готовый массив `catalogFacets` и рендерит `x-catalog.title-filters` внутри автоматически deferred island. Компонент не выполняет запросы; checkbox/select используют `wire:model.live`, JavaScript-submit скрыт в `<noscript>` fallback.
...
- `x-ui.poster-frame` — единственный Blade boundary для `<img>` каталожного постера: готовый URL и alt передаются вызывающим слоем, изображение использует cover + 2% overscan, а background появляется только у заглушки.
- `x-ui.poster-card` собирает один внешний shell и poster frame в строгих layout `grid`, `horizontal`, `compact`; он не создаёт wrapping anchor, поэтому основной stretched-link и независимые действия остаются корректными.
