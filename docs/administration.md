...
`/admin/catalog` — единственный full-page Livewire shell для управления сериалами и коллекциями и защищён stable permission `content.view`; mutations разделены между `content.*`, `sources.*`, `collections.moderate` и `recommendations.manage`, а `manage-catalog` остаётся compatibility alias для `content.manage`. Query-параметр `section=collections` требует `collections.moderate` и монтирует `CatalogCollectionAdministrationManager` как вложенный manager без собственного маршрута; `catalog_q`, `collection_admin_q`, `catalogAdminPage` и `collectionAdminPage` не конфликтуют. Manager показывает bounded pending/open-report queue, а `CatalogCollectionModerationService` является единственной write boundary для approved/rejected/hidden/archived и feature. Пользователь с `content.manage` дополнительно управляет двухуровневым category dictionary: stable slug/UUID, `ru|en` names, order и archive/restore. Create form и пять root branches закрыты native `<details>` и не требуют JavaScript. Каждое material action повторно разрешает stable UUID и locked record, пишет category/moderation audit в той же transaction и инвалидирует discovery/cache/sitemap after commit; browser не создаёт произвольную третью глубину и не передаёт «все строки».

Центр классификации в этом же manager показывает реальные total/categorized/uncategorized/public-uncategorized counts, поиск, visibility/moderation/type и отдельную пагинацию очереди. Начальное состояние ограничено public/approved rows. Детерминированная подсказка вычисляется только для текущей страницы, показывает score/confidence и до трёх локализованных причин; category каждой строки редактируется вручную. «Выбрать страницу», «Выбрать с высокой уверенностью», «Принять рекомендацию» и batch category меняют только текущий draft и ничего не сохраняют; confidence ordering также не выходит за текущую страницу. Запись не более 100 точных UUID выполняется лишь после отдельного preview и подтверждения; изменившийся `content_version`, уже назначенная строка или inactive category безопасно не превращаются в скрытое автоматическое назначение. Request-scoped computed не дублируют tree/page/summary/suggestion reads, а единая manager island синхронно обновляет строки, счётчик, preview и справочник.
...
Field-level исправления создаются только администратором с `manage-content-requests` и показывают в той же очереди конкретное поле, безопасное текущее и предложенное значение и, для тега, одну из пяти стабильных причин. Они всегда private, не получают vote/follow, не выводятся в public/My Requests, поиске, sitemap или пользовательских уведомлениях. Переходы `approved` и `rejected` подтверждают либо отклоняют предложение с прежней optimistic version/history boundary; approval не выполняет скрытую запись в каталог, а оставляет фактическое исправление существующей редакционной/import action с собственным audit и invalidation.
...
Та же private/no-store очередь содержит отдельную агрегированную диагностику качества просмотра за allowlisted периоды 1, 7 или 30 дней. Четыре итоговые метрики считаются server-side: средний startup time; rebuffer ratio как `buffering / (playback + buffering)`; terminal playback error rate как `failed sessions / all sessions`; fallback success как `successful fallbacks / attempted fallbacks`. Три таблицы ошибок ограничены десятью группами по сокращённому browser family/major, server-owned provider code и quality code. Страница не выдаёт playback rows, request IDs, пользователей, IP, raw UA, source URL, private ticket diagnostics или provider credentials; доступ повторно защищён `support.tickets`.
