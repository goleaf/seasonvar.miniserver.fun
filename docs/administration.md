...
Field-level предложения показывают в той же очереди конкретное поле, безопасное текущее и предложенное значение и, для тега, одну из пяти стабильных причин. Переходы `approved` и `rejected` подтверждают либо отклоняют предложение с прежней optimistic version/history boundary; approval не выполняет скрытую запись в каталог, а оставляет фактическое исправление существующей редакционной/import action с собственным audit и invalidation.
