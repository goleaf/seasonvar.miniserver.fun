...
Task 119 изменил гостевой HTML главной без изменения snapshot/API/query
данных, поэтому `PublicPageCachePolicy` использует homepage
`response_contract=3`. Старые envelopes с presentation contract `2`
становятся недостижимыми по точной dimension без wildcard scan, broad flush
или user-specific key. Authenticated home, Livewire requests и private state
по-прежнему обходят shared full-response cache. Rollback требует
согласованного возврата Blade/assets и response contract; migration,
database restore и очистка всего cache store не нужны.
