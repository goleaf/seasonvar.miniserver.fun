...
После успешной активации job записывает существующий catalog warm request и запускает `WarmCatalogCaches`; payload/page tiers продолжают использовать настроенные Redis/Memcached stores, а locks/unique state — выделенный lock store. Complete source snapshot удаляет stale membership/signals и инвалидирует затронутые scopes, partial snapshot сохраняет прежние связи и не создаёт destructive cache transition. Public карточка всегда читает локальный WebP через authorization-aware `private, no-store`, поэтому CDN/browser immutable cache для imported covers не допускается.
