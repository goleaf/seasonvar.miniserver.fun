...
- `settings_version` и versioned key `seasonvar.account-preferences.v1` разрешают device state без email/user ID в key. Opaque HMAC owner scope изолирует accounts в одном browser; account version не позволяет старому local state бессрочно перекрывать новый server choice.
