...
Поиск работает только по опубликованным тайтлам. Для состояния `ready` полная значимая фраза сначала сравнивается с точным основным и оригинальным названием, а алиасы проверяются по `exactNameHashes`. Если найдены точные ID, широкий текстовый поиск не выполняется. Иначе каждый значимый терм должен совпасть с одним и тем же тайтлом: внешние группы термов соединяются через `AND`, а варианты основного и оригинального названия, описания, slug, внешнего ID, алиасов и имен связанных справочников внутри терма соединяются через `OR`.
...
На первом проходе `CatalogTitleQuery` состояние `insufficient` добавляет нулевое условие, если `title` context отсутствует. Для существующего title-scoped запроса сохраняется исключение: при заданном допустимом `title` остается его `whereKey()`-ограничение. На текущей публичной странице нулевой результат первого прохода без распознанного года еще может быть заменен временной fallback-выдачей опубликованного каталога, как описано в ограничениях rollout ниже. Все варианты сортировки заканчиваются `catalog_titles.id DESC`, поэтому равные основные поля не меняют порядок между запросами.
...
До следующего этапа rollout сохранен прежний fallback для нулевого запроса без распознанного года, включая unscoped-запрос в состоянии `insufficient`. После первой пустой выдачи page builder не разбирает строку повторно, а создает один явный объект состояния `empty` и использует его для fallback-результатов и контекстных счетчиков. Поэтому текущий HTTP-ответ может показывать опубликованный каталог, хотя первый проход `CatalogTitleQuery` получил нулевое условие. Этот временный путь не применяется к запросам с распознанным годом и конфликтам годов. Удаление fallback и связанного UI/SEO-состояния выполняется отдельно.
