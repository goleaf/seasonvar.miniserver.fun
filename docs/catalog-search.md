...
## Crawl budget и ленивая панель фильтров
...
Первый GET и новый текстовый поиск не вычисляют relation/year/publication/subtitle facets и не отправляют сотни скрытых checkbox в Livewire HTML. `CatalogSeries::$facetsLoaded` становится `true` только через `loadFacets()`; desktop показывает компактную кнопку, mobile trigger одновременно открывает существующий native dialog и запускает загрузку. После нового поиска состояние снова сбрасывается.
