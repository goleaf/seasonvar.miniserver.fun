...
- `GET /api/v1/search/suggestions?q=...` принимает 1–80 нормализованных символов. Без scope ответ сохраняет legacy-форму и максимум по пять тайтлов, актёров и режиссёров; односимвольный запрос выполняет только exact-short title lookup. Allowlisted `scope=header_titles` даёт public-only карточки с URL, постером, основным и оригинальным названием, годом, числовыми счётчиками и готовой локализованной `meta` для сезонов/серий, а `scope=header_portal` требует минимум два символа и возвращает сгруппированные имена и URL публичных справочников, годов, подборок, заявок, профилей и разделов. Web-header отправляет поддерживаемый язык текущего `<html lang>` в `Accept-Language`; другие клиенты используют тот же документированный API locale contract. Новые optional fields проходят только через `SearchSuggestionResource`; description, private fields, raw remote URL и search internals отсутствуют.
...
- `GET /api/v1/me` возвращает `UserResource`; `PATCH /api/v1/me` меняет только имя/email, а реальная смена email дополнительно требует `current_password`. Она сбрасывает `email_verified_at`, удаляет reset rows обоих адресов и отправляет новое подтверждение; смена имени через тот же `AccountService`, что и Livewire, обновляет versioned public-profile/search и identity-dependent projections. `PATCH /api/v1/me/password` требует текущий пароль, удаляет reset token и при Bearer-аутентификации отзывает все device tokens, кроме текущего; допустимый first-party session-вызов сохраняет текущую browser session и отзывает все mobile tokens. `DELETE /api/v1/me` требует пароль и удаляет account tokens/reset/session/private owner state/morph notifications через общий lifecycle; immutable admin-audit retention возвращает безопасную validation error до mutation.
...
- Внутренний pull boundary единообразен для catalog и owner scope: `ApiSyncPullService` оркестрирует checkpoint, decode, bounded query и encode и возвращает типизированный `ApiSyncPullResult`. `SyncController` сохраняет только readiness/auth, безопасное отображение ошибок, API Resource и HTTP headers; transport contract и формат cursor не изменены.
...
- `GET /api/v1/tags?q=` возвращает максимум 25 eligible canonical tags: при query от двух символов — canonical/translation/approved-alias search с de-duplication и bounded synonym expansion, без query — popular by distinct visible serial count. Обе compact projections возвращают одинаковый optional stable `code`; cache projection version изолирует прежний payload, где popular branch мог отдавать `null`. Private/internal/unapproved/archive/empty rows исключены.
...
- `GET /api/v1/me/tags` owner-only ищет собственные active personal tags. Verified `POST /me/tags`, `PATCH|DELETE /me/tags/{uuid}` и `POST /me/tags/{uuid}/restore` создают, optimistic-edit-ят, soft-delete-ят и восстанавливают private tag. Owner ID/type/visibility/moderation не принимаются; label остаётся на исходном языке. Optional `content_locale` допускает только explicit nullable `ru|en`: отсутствие поля при PATCH сохраняет прежнее значение, явный `null` очищает его, а locale интерфейса никогда не подставляется как язык UGC.
