...
- `AccountService` является общей Livewire/API boundary для изменения name/email/password и удаления account. Смена name внутри сервиса обновляет versioned public-profile/search projection и identity-dependent collection/comment/review caches для обеих границ. Удаление очищает owner notifications вместе с sessions/tokens; если actor связан с immutable `admin_audit_events`, hard deletion fail-closed останавливается локализованной retention validation до изменения данных, потому что audit FK намеренно `RESTRICT`.
...
`CatalogCollectionPolicy` — единственная authorization boundary коллекций. `view` разрешает approved public/unlisted или owner/admin и иначе отвечает `denyAsNotFound`; private name, count, owner и canonical slug не раскрываются. `create` требует authenticated verified user; `createEditorial` и изменение system/editorial content требуют `content.manage`, а moderation/feature — отдельный `collections.moderate`. Update/delete/restore/force-delete/item/reorder повторно разрешают stable resolved record; cover action/route отсутствуют. Drag reorder передаёт только item ID/position: service повторно проверяет ownership, item membership и current/target page window под collection lock, поэтому DOM order не является authority. System record изменяет только content manager, а feature разрешена только approved public editorial collection.

Owner берётся из authenticated actor внутри service; client не передаёт `owner_id`, type/moderation/feature не mass-assignятся из public form. Category selector принимает stable UUID, но сервис повторно разрешает активный root/child; public/unlisted owner write без категории отклоняется. Только `content.manage` создаёт, переводит, сортирует, архивирует и массово назначает до 100 exact collection UUID. Title ID повторно проходит `CatalogTitlePolicy::interact`/visible query, collection UUID batch сравнивается с полным owner-scoped manageable set, item IDs проверяются внутри locked collection. Report разрешён verified non-owner только для directly public-viewable target и имеет отдельный limiter/deduplication key; moderator decision повторно проверяет gate уже после row lock, поэтому stale Livewire payload не обходит актуальную authority/state boundary.

Classification suggestion, confidence, browser selection и locked preview являются только интерфейсным состоянием, а не authorization evidence. Подтверждение заново требует `content.manage`, валидирует полный пакет из 1–100 уникальных UUID, lock-ит authoritative rows и применяет только всё ещё не классифицированные записи с совпавшим `content_version` к активной категории с активным root. Неизвестный или inactive target отклоняет пакет атомарно; stale и уже назначенные записи считаются пропущенными без изменения защищённых полей.

Private и pending/rejected/hidden/deleted records не дают redirect/canonical metadata постороннему: slug history resolution всегда завершается policy до `301`. Unlisted — direct-link visibility, не access token и не секретное хранилище; owner management всё равно policy-protected. Collaborators/ownership transfer/follows/collection likes отсутствуют, поэтому не симулируются UI-флагами.
...
`/profile/reviews` and review preference/history actions use only current authenticated user. `/admin/reviews` has route gate plus component/action policy. Public `/reviews/{review}` and `/{locale}/reviews/{review}` accept only a positive stable ID, resolve alias, reauthorize review and current title, then redirect through the same responder; hidden/deleted/blocked state returns safe not-found and cannot be used as an IDOR oracle. Task 14 public-profile reviews additionally require the profile's explicit public section decision and then reuse only the canonical published-review/visible-title query; list and count both apply one bounded block/mute context, while moderators retain their canonical visibility. No owner or moderation overlay is exposed. Review restriction affects only review create/edit/restore/vote/report and never login, comments, playback, rating, watchlist, progress, collection or account deletion.
