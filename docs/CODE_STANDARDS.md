...
- Карточка тайтла получает summaries/counts и active-season episodes через `CatalogTitlePlaybackQuery`; нельзя возвращать eager loading всех серий всех сезонов или дублировать playback availability в Blade/Livewire.
- Private watchlist/rating/progress writes проходят policy и `CatalogUserStateService`, а `catalogTitleId` Livewire-компонента остаётся locked; browser ID всегда повторно разрешается внутри доступной иерархии.
