...
`DatabaseSeeder` fail closed вне `local|development|dev|testing`: он не
создаёт известные local/demo credentials и не запускает demo graph в
production. В среде `dev` обычная команда `php artisan db:seed` после
локальных `admin@example.com` и `user@example.com` запускает
`PortalDemoSeeder`. Он создаёт или обновляет ровно 100 подтверждённых
пользователей `user1@example.com`–`user100@example.com` с паролем `password`
и детерминированно заполняет пользовательские состояния ровно для половины
опубликованного каталога на каждого пользователя.
