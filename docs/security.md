...
- Общий `throttle:catalog-stats` защищает Livewire transport, а `SensitiveActionRateLimiter` дополнительно разделяет бюджеты поиска, playback-session, progress, rating, watchlist, history и import-admin по authenticated user и хэшу ресурса. Source-health ограничивается по хэшу host; исчерпание локального бюджета дает `not_checked`, не считается отказом провайдера и не раскрывает host в cache key.
