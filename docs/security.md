...
## GitHub supply-chain boundary

- Repository policy разрешает GitHub-owned actions и только exact external ref `shivammathur/setup-php@f3e473d116dcccaddc5834248c87452386958240`; `sha_pinning_required=true` запрещает mutable action refs. Workflow сохраняет явное `contents: read`, default token не может одобрять pull request, а checkout credentials не сохраняются.
- Secret scanning и push protection включены на стороне GitHub. Passive Dependabot vulnerability alerts включены 20.07.2026; read-only проверка в этот момент вернула `0` открытых alerts. Это датированный сигнал, а не гарантия отсутствия неизвестной уязвимости.
- Автоматические Dependabot security updates не включены, потому что создаваемые ими pull-request branches конфликтуют с обязательной единственной веткой `main`. Каждое обновление продолжает проходить отдельный advisory, compatibility, rollout и rollback review.
- Ruleset `Protect main history` блокирует удаление `main` и non-fast-forward rewrite. Обычная отправка в `main` остаётся возможной только после локального pre-push gate; отсутствие обязательного server-side pre-merge check честно зафиксировано как ограничение direct-to-`main` процесса.
...
- IDOR: public URL использует slug→stable record, management/membership/category assignment — opaque UUID; every view/mutation re-resolves and authorizes. Unauthorized private/pending/deleted targets return safe 404 before metadata.
...
- Upload: у подборок нет upload/delivery route, image metadata, responder, fallback URL или image cache. Legacy cleanup доступен только через operator-команду `catalog-collections:purge-covers`: dry-run по умолчанию, явный `--execute`, жёстко заданные disk/prefix, bounded database batches, агрегированный вывод без private paths и schema guard перед удалением колонок.
- Privacy/cache: private membership, owner controls, report/moderation notes и personal state never enter public cache/API/structured data. Visibility/moderation/category changes bump every affected domain after commit; a fulfilled retry reissues derived invalidation if that post-commit step previously failed, while direct policy/query truth never trusts cache. Collection API/sitemap have zero shared/stale TTL, а page responses are no-store.
- Smart rules: mode разрешён только server-derived owner-owned `user/private` collection. `CatalogSmartCollectionRules` принимает фиксированный набор scalar keys и enum values; browser не управляет owner, SQL column/operator или raw JSON. Неизвестные/malformed stored rules fail-closed. Public scopes дополнительно требуют `mode=manual`, account export не materialize-ит private result, а ICS повторно связывает collection с владельцем token. Все manual item mutations отклоняют smart mode под database lock, даже если вызваны вне скрытого UI.
- Abuse/concurrency: create/mutate/category assignment/membership/reorder/report имеют user-scoped RateLimiter budgets. Stable UUID create tokens make retry idempotent; collection/item mutations re-authorize locked current rows, add/batch revalidate current title entitlement, and DB locks, unique item/report keys, optimistic `content_version` plus transaction retries prevent duplicate submit/stale edit/order races. Report submission re-authorizes locked current visibility and atomically inserts its content-version key. Moderation/feature/report resolution re-authorize locked state, commit audit atomically, treat completed exact retries as no-op and bound bulk collection/category/report actions to 100 rows per request.
- Destructive lifecycle: no GET mutation exists. Soft delete revokes access immediately; permanent delete is owner/admin or scheduled after the documented window, removes only structural rows, preserves report evidence and privacy-retires generic comments without touching catalog titles or personal viewing state. Legacy collection images не восстанавливаются и удаляются только exact-prefix purge-командой по отдельному явному operator-действию.
...
- Username change requires current password, an accumulating `5/hour` limiter, reserved/format/current+history uniqueness and a locked transaction. Same-name retries verify the password but neither mutate identity nor erase earlier limiter attempts; a concurrent already-applied desired username does not increment the content version twice. Biography/report/private moderation prose uses the canonical Unicode plain-text sanitizer and rejects control/surrogate/bidi input at the Livewire boundary. Client-supplied arbitrary fields/privacy keys/status/role/badge are never mass-assigned.
- Avatar/cover accept bounded JPEG/PNG/WebP source MIME/extension/dimensions/pixels only; SVG/executables are rejected. The server applies JPEG EXIF orientation, center-crops/resamples to the allowlisted design size (`320×320` avatar, `1280×360` cover), re-encodes WebP and therefore strips source metadata/client filename. Raw paths stay private, storage names are generated server-side and cleanup is scoped to the configured owned upload disk.
- Reports require verified non-owner access, stable allowlisted category, bounded detail, rate limiting and unresolved deduplication. Moderator actions require `manage-catalog`; private notes/reporter identity never cross the admin boundary. Canonical portal suggestions search only active public username/display name and fail closed while profile schema is incomplete; email, biography, private profiles and moderator data never enter the shared result. Follows/badges/ranks and a standalone profile directory remain absent rather than insecure partial implementations.
...
- Exact viewing positions, private tag text, settings, notification action/read state, sessions и tokens никогда не попадают в shared HTML/cache. Owner `user-portal` cache допускает только versioned aggregate/ID arrays под stable public UUID; authenticated HTML остаётся `private, no-store`, а hydration повторно ограничивается current owner/visibility query. Task 14 public profile использует отдельный allowlisted DTO и private versioned media delivery; OAuth/external identity, premium/billing и MFA по-прежнему отсутствуют, поэтому интерфейс не показывает фиктивные provider/entitlement controls.
...
- Personal URL не принимает user ID. Livewire получает current authenticated user server-side; type/source/reason/feedback/relation/filter/provider/limit проходят enum/allowlist/bounds validation. Feedback/watch status повторно authorizes `interact`, editorial relations — `manage-catalog`; mutation идёт только через CSRF-protected Livewire и rate limiter, GET остаётся read/redirect. Query-free feedback component принимает только positive title ID и одно из двух внутренних allowlisted action names, поэтому dynamic Livewire directive нельзя превратить в инъекцию.
...
- Public recommendation API намеренно игнорирует authenticated
  personalization. Authenticated, personalized и random results bypass shared
  cache. Deterministic guest refresh может читать тот же канонический scalar
  pool, что и обычный guest request, но session recent IDs фильтруются только
  после cache lookup. Поэтому seed/recent IDs, user ID и email отсутствуют в
  shared key/value, а hash содержит только публичный canonical exclusion set.
  Cache/analytics failure не блокирует rendering. Отдельная impression/click
...
- Account export includes stable explicit `more_like_this|not_interested|blacklisted|null` feedback/status; account deletion removes user-title rows and session suppression through existing lifecycle and bumps the public recommendation version because watchlist/rating activity aggregates changed. Public similarity/editorial/catalog data не удаляется вместе с account.
...
Anonymous playback telemetry является отдельным минимальным operational aggregate без account linkage: таблица не имеет `user_id`, IP и raw user-agent columns. Encrypted expiring capture token содержит только title identity; server повторно разрешает `media_id` внутри canonical title/season/episode hierarchy и сам извлекает provider/variant/quality/translation/format. Form Request allowlist-ит event/error/browser/OS/HLS/network status, жёстко ограничивает числовые counters и игнорирует неизвестные поля. CSRF, global opaque network limiter и request-ID limiter ограничивают запись; request UUID не используется как authorization secret. Ответы private/no-store/noindex и не содержат source URL, storage path, grant или provider credential.

Explicit network test не является SSRF или speed test: маршрут не принимает target URL/body, не выполняет outbound request и отвечает фиксированным same-origin `204`. Client измеряет только bounded round-trip после клика, не использует Network Information API и не сохраняет bandwidth/provider/IP. Server-issued report token зашифрован и ограничен сроком; snapshot копируется в приватное техническое обращение только после authenticated owner flow, повторной проверки title/media hierarchy и оставленного пользователем consent. Telemetry никогда сама не меняет source health или доступность источника.
...
Приватный `/calendar/feed/{private-token}.ics` является capability URL, а не session route. 48 random bytes кодируются в 64 base64url-символа; база ищет только unique SHA-256 hash, encrypted secret доступен только владельцу в закрытом Livewire UI и никогда не входит в export/log/cache. Route не запускает cookie/session/CSRF middleware, ограничен token+IP limiter, повторно проверяет blocking account restriction и canonical title/media visibility, а invalid/revoked/foreign/deleted target одинаково возвращает `404`. Ответ всегда `private, no-store`, `noindex,nofollow,noarchive`, `nosniff` и `no-referrer`; регенерация под row lock немедленно отзывает старую ссылку.

Google action передаёт токен только same-origin clipboard и открывает официальный экран добавления по URL с `noopener,noreferrer`; произвольного redirect/prefill нет. Apple получает локально сформированный `webcal://` URL. JavaScript допускает только текущий origin и точный token path, не использует `innerHTML`, storage или third-party request. Owner create/regenerate/delete повторно scope-ятся по authenticated user и имеют отдельный mutation limiter; скрытая кнопка не заменяет policy/service boundary.
