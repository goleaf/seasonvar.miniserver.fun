...
Global admin form принимает только enum-backed type/visibility/moderation/source, safe canonical label/slug, optional code, фиксированную русскую translation row, alias/synonym/provider stable IDs и explicit merge target. RU/EN controls и подпись «Русский» отсутствуют; новый alias получает `ru`, существующие non-Russian rows сохраняются. Create imported/hidden public, code mutation, lifecycle state spoofing, alias conflict/self synonym/invalid merge блокируются service. Merge/archive/title assignment требуют confirmation/impact и per-action loading state; private personal tags не являются options.
