...
- Все фильтры `/titles` находятся в одной GET/Livewire-форме внутри полноширинного `<details id="catalog-filters">`. Summary показывает общее число активных условий и раскрывается при любом из них; отдельные mobile dialog и desktop sidebar отсутствуют.
