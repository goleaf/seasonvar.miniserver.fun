...
1. Deploy rolling-safe code и примените additive `2026_07_16_200000_create_technical_issue_domain.php`, затем `2026_07_16_200100_add_requester_order_index_to_technical_issues.php`; первая migration создаёт только 13 новых таблиц, вторая — только covering index основного requester-list. До базового DDL `TechnicalIssueSchema` показывает безопасный unavailable state. Не редактируйте старые migrations и не выполняйте `migrate:fresh`/`db:wipe`.
