...
Гостевой full-response cache dimensioned `Vite::manifestHash()`: переключение manifest выбирает новое HTML generation без store-wide flush. Это страховка от stale HTML, а не разрешение удалять активные assets раньше времени: manifest и все referenced files публикуются согласованно, предыдущий набор сохраняется до активации и smoke-проверки, после чего PHP-FPM перечитывает новый code/config.
...
Unit слушает только `cache-warm-v2`: job и worker используют timeout 600 секунд, `TimeoutStopSec=660`, а Redis `retry_after=1200` оставляет запас до повторной доставки. Heartbeat lease этой очереди вычисляется как `max(CACHE_QUEUE_WORKER_HEARTBEAT_SECONDS, CACHE_WARM_TIMEOUT + 60)`; обычные queue pools сохраняют короткий базовый lease. Поэтому разрешённая долгая `WarmCatalogCaches` не выглядит остановленным worker, а аварийно завершённый cache-warm process остаётся обнаружимым после ограниченного окна. Быстрый критический self-HTTP проход имеет отдельный общий бюджет 240 секунд; после его исчерпания оставшиеся цели отмечаются как пропущенные, health становится `degraded`, а cold/stale request path продолжает обслуживать их без flush или retry-шторма. После запуска дождаться обработки текущего versioned intent, проверить heartbeat/warming state, `cache:metrics --json` и повторные guest HIT. При росте SQLite wait/CPU/ошибок остановить unit штатно; pending intent сохранится и не требует flush. Старую `cache-warm` не включать в новый worker и не удалять автоматически: её disposition остаётся отдельной операторской процедурой после сверки агрегатной сводки failed/pending jobs.

### Repair демонстрационных пользовательских разделов

`demo:repair-user-portal` не заменяет production seeding и не принимает произвольных пользователей. Сначала выполнить только `php artisan demo:repair-user-portal --dry-run --json`, проверить точное число известных demo accounts, свободное место, очередь `cache-warm-v2`, отсутствие активного импорта и текущие invalid image/request counters. Перед записью остановить web/CLI/queue writers штатными средствами реально установленной инфраструктуры, создать согласованный SQLite backup вне публичного и tracked storage, проверить backup через `PRAGMA quick_check`, foreign keys, размер и контрольную сумму. Только затем разрешён `php artisan demo:repair-user-portal --force --backup-confirmed --writers-paused --json`.

После repair нужно повторить dry-run: `users_without_library`, `users_without_personal_tags`, `users_without_requests` и `invalid_profile_images` должны быть равны нулю; collection-image counter отсутствует, потому что подборки не имеют image domain. Затем запустить writers, проверить worker `cache-warm-v2`, открыть `/requests`, `/library`, `/library/tags/manage`, `/profile` и collection section `/discover/popular` под demo account, убедиться в отсутствии `404` у versioned profile media и в text-only карточках подборок. При partial failure не выполнять `cache:clear`, `queue:clear` или повторный полный seed: команда идемпотентно повторяется только после диагностики и нового backup. Data rollback — восстановление проверенной SQLite backup при всё ещё остановленных writers; после появления новых пользовательских записей предпочтителен roll-forward, чтобы их не потерять.

Для прогрева видимых карточек `/titles` примените additive migration индекса пользовательского title-summary после проверенной резервной копии и оценки writer pause, затем пересоберите config/routes, выполните graceful reload PHP-FPM и перезапустите единственный `cache-warm-v2` worker, чтобы long-lived процессы получили `WarmCatalogTitlePage`. Defaults: `CACHE_VISIBLE_TITLE_WARM_ENABLED=true`, максимум 96 ID, import pause 300 секунд, cache-outage pause 60 секунд, retry window 86 400 секунд и unique window 86 700 секунд. Уже поставленные Laravel payload сохраняют прежние `maxTries=3`, `retryUntil=null` и старый unique lock; не переписывайте и не очищайте их. Они безопасно завершаются best-effort, а следующий показ `/titles` после истечения старого lock создаёт новое задание по актуальному контракту. После выпуска один guest `GET /titles` должен дать `MISS` нового response contract или `HIT` уже нового payload, а несколько реально видимых `/titles/{slug}` после обработки очереди — `HIT`; URL или cache keys в операционный журнал не записываются.

При росте backlog/SQLite wait сначала установите `CACHE_VISIBLE_TITLE_WARM_ENABLED=false`, пересоберите config и graceful-restart PHP-FPM/worker. Не выполняйте `cache:clear`, `queue:clear` или массовый retry: уже созданные jobs идемпотентно завершатся либо будут объединены locks, а запросы продолжат cold/stale path. Application rollback сохраняет новый индекс; снять его отдельным `migrate:rollback` можно только после backup и оценки блокировки, но для восстановления кода это не требуется.

Полный проход по всему безопасному публичному реестру выполняется только вручную и тем же единственным worker. Перед первым production-запуском обязательно выполнить dry-run, зафиксировать число целей и проверить свободную память Redis, backlog `cache-warm-v2`, отсутствие активного импорта и состояние SQLite. Затем поставить поколение в очередь и наблюдать отдельный компонент `full_cache_warming`:
...
Начинать следует с тестового окружения и ограниченного наблюдаемого прохода, затем разрешать полный production-проход. Активный импорт сам откладывает следующий пакет; при остановке worker cursor сохраняется, а продолжение выполняется `php artisan cache:warm-catalog --scope=all-public --queue --resume`. Не используйте `cache:clear`, queue cleanup или массовый retry. `completed_with_failures` требует анализа безопасных HTTP status/class агрегатов и нового полного поколения, а не раскрытия URL из журналов. Профили, личные коллекции, account/API-me, admin, signed playback/download, auth, поиск и другие чувствительные или персонализированные маршруты намеренно не прогреваются.
...
- `SEASONVAR_TITLE_REFRESH_QUEUE=seasonvar-title-refresh` — отдельная очередь browser-triggered групп; `SEASONVAR_TITLE_REFRESH_FINALIZER_DELAY_SECONDS` задаёт начальную задержку finalizer, но не polling и не ограничение page jobs. `SEASONVAR_QUEUE_FINALIZER_WATCHDOG_BATCH_SIZE=250` ограничивает десятиминутное восстановление потерянных terminal signals. `SEASONVAR_IMPORT_*_RETENTION_DAYS` задают окна events/snapshots/terminal staging groups, `SEASONVAR_IMPORT_MAINTENANCE_CHUNK_SIZE`, `..._MAX_CHUNKS`, `..._MAX_ROWS` и `..._TIME_BUDGET_SECONDS` — общий bounded проход. Независимый daily dispatch остаётся выключенным через `SEASONVAR_IMPORT_STORAGE_MAINTENANCE_SCHEDULED_ENABLED=false`.
...
- Code-preparation регистрирует internal daily job `seasonvar-import-storage-prune` на configured `04:17`, но default schedule switch равен `false`. Job не принимает data payload, не вызывает provider и ограничен общими `max_chunks`, `max_rows` и monotonic time budget; active runs/groups и latest snapshot не удаляются. Safe preview возвращает только агрегатные counts/bytes/timestamps/status counts. Это не доказательство готовности production: до включения нужны независимый проверенный backup и restore rehearsal, один подтверждённый scheduler owner, отсутствие неожиданных writers, записанный preview и один минимальный canary с проверкой latency, WAL и размера.
- Первый rollback switch — вернуть `SEASONVAR_IMPORT_STORAGE_MAINTENANCE_SCHEDULED_ENABLED=false`, пересобрать config cache и штатно перезагрузить scheduler/worker. Уже удалённые rows не восстанавливаются code rollback и требуют backup; `VACUUM`/compaction, index build, failed-job cleanup, queue/cache clear и изменение retention windows не входят в canary. Ошибка между chunks оставляет завершённые deletes и безопасно повторяет оставшийся eligibility set.
...
Базовый collection rollout состоит из `2026_07_15_200000_create_catalog_collections.php` и `2026_07_15_200100_create_catalog_collection_translations.php`. Текущий rollout таксономии последовательно применяет additive `2026_07_25_140000_create_catalog_collection_categories.php`, `140100_add_category_to_catalog_collections.php` и idempotent `140200_install_default_catalog_collection_categories.php`. Они создают пять root и 31 child с `ru|en` переводами, но намеренно не классифицируют существующие подборки по названию. Existing watchlist/progress/history/catalog/comment/review data не переписывается. Перед `migrate --force` применяются обычные production backup, writer pause и SQLite integrity rules этого документа.
...
Удаление прежних изображений выполняется перед destructive schema migration отдельной границей: сначала `php artisan catalog-collections:purge-covers` в dry-run, затем только при проверенном backup/database state и явном подтверждении продуктового требования — `php artisan catalog-collections:purge-covers --execute`. Команда ограничена точным prefix `uploads/catalog-collections/`, не создаёт backup/trash-копию, очищает legacy metadata bounded-пакетами и не выводит private paths. Только после повторного zero-residue dry-run применяется `2026_07_25_140300_drop_catalog_collection_cover_columns.php`. Её `down()` возвращает пустые колонки, но не удалённые файлы или metadata; media rollback невозможен.

Центр классификации не добавляет migration, dependency, environment variable, provider, queue или scheduler и не запускает фоновую классификацию. После code/assets rollout проверьте под `content.manage` summary, поиск/visibility/type, 10/20/30/50 pagination, локализованные причины, выбор уверенных только на текущей странице, ручную замену category и preview/cancel без записи. Отдельный контролируемый confirm должен изменить только выбранные uncategorized rows, увеличить их `content_version`, записать audit и обновить `/discover/popular`; stale preview обязан дать skipped result. Rollback кода удаляет cockpit, но уже подтверждённые category FK являются обычными редакционными данными и автоматически не отменяются: при необходимости их восстанавливают только из verified backup или отдельной авторизованной reconciliation, без broad SQL и global cache flush.

После migrations пересоберите config/routes/views и production assets, graceful reload PHP-FPM/Livewire workers, затем проверьте `/discover/popular`, owner dashboard, public/unlisted/private detail, category/subcategory filters, title selector, `/api/v1/collections`, `/sitemap-collections.xml` и account export. Bare `/discover`, отдельный `/collections` directory и прежний cover route должны отвечать `404`; detail `/collections/{slug}` остаётся рабочим. Public collection sitemap включён в существующий sitemap index; collection API/sitemap HTTP profiles должны иметь нулевые shared/stale TTL. Не прогревайте private/unlisted pages. Собственных collection images, delivery response или CDN image cache больше нет.
...
2. Убедитесь, что Redis lock store доступен, а worker слушает configured `HDREZKA_COLLECTION_RECOMMENDATION_QUEUE` (по умолчанию существующая `seasonvar-import`) и `cache-warm-v2`. Collection sync больше не зависит от GD/WebP и не записывает `catalog-collections` storage prefix. Задайте лимиты/очередь из `.env.example`, включите flag, пересоберите config cache и graceful-restart PHP-FPM/workers; `.env` и credentials не коммитятся.
...
Discussion rollout применяет по порядку четыре базовые additive migrations — `2026_07_15_210000_create_comments_table.php`, `210100_create_comment_engagement_tables.php`, `210200_create_discussion_preference_tables.php`, `210300_create_notifications_table.php` — затем focused reversible `2026_07_15_235200_add_discussion_relationship_pagination_indexes.php`, idempotent `2026_07_18_193800_repair_removed_comment_deletion_state.php` и `2026_07_18_193801_add_public_count_index_to_comments_table.php`. Последние добавляют только covering `(blocker_id,id)`, `(muter_id,id)`, exact public-count `(target_type,target_id,status,deleted_at)` paths и заполняют отсутствующие deletion metadata исключительно у уже `status=removed` rows. Повторный preflight на 3 707 221 comments/9 265 514 reactions подтвердил отсутствие duplicate reactions/submission keys, invalid enum, orphan/cross-target/deep replies и competing tables; destructive reconciliation не выполняется. Existing provider reviews, catalog, collections, watchlist/rating, progress/history и import rows не переписываются.

Порядок: verified backup и writer/worker pause по общему SQLite runbook; убедиться в достаточном свободном месте для нового индекса; deploy code; до migration записать количество `removed` rows с отсутствующими reason/deleted timestamp; `php artisan migrate --force`; подтвердить, что только эти rows получили metadata и их body/ID/reply/reaction/report counts неизменны; проверить `PRAGMA quick_check`, foreign keys, наличие `comments_target_public_count_idx` и exact-count query plan; rebuild `config:cache`, `route:cache`, `view:cache`; `npm run build`; graceful PHP-FPM reload. Создание индекса на 3,7 млн строк не объявляется online/zero-downtime и не выполняется при активных writers. PHP-FPM reload обязателен также для единообразной загрузки нового locked direct-focus/noindex behavior всеми long-lived workers. Checked-in/local route cache может быть старше source routes, поэтому post-deploy route cache rebuild обязателен. Затем read-only проверить `comments.show`, оба `localized.comments.show` (`ru|en`), focused anchor в initial HTML, clean canonical/`noindex`, `profile.discussions`, `notifications.index`, `admin.comments`, title/season/episode/collection scope, public-profile exact-target/block/mute behavior и отсутствие comments в sitemap/OpenAPI.
...
Повторное усиление 18.07.2026 добавляет только idempotent data repair `2026_07_18_220000_repair_removed_review_deletion_state.php`. Изначально она оставалась `Pending`; 19.07.2026 другой rollout применил её вместе с comment repair/index в batch 31. Финальная read-only сверка подтвердила ноль несовместимых removed review tombstones и ноль foreign-key violations. Merged tombstone требует `merged_into_id` и `ownership_released_at`, но не moderator actor; actor обязателен только для author/moderator deletion, поэтому nullable actor у merged rows не ремонтируется. Доступный pre-migration backup artifact batch 31 аудит Task 29 не нашёл и наличие backup не заявляет. Для любого следующего rollout до DDL всё равно необходимо остановить review/admin/demo/title-merge writers, создать и проверить backup, записать точный affected set и затем подтвердить, что изменились только nullable evidence fields, а body/ID/title/ownership/vote/report/alias counts, `PRAGMA quick_check`, foreign keys и оба review responders сохранились. `down()` не обнуляет восстановленное evidence; rollback выполняется возвратом совместимого code или forward fix, не data corruption. Global cache flush и новый worker не требуются.
...
5. Smoke current/case/history/alias/merge redirects, ambiguous alias fail-closed, empty/ineligible 404, `/tags`, `/titles/tag/{slug}`, sitemap taxonomy entry, public API/search/suggestions, title badges, filters/sorts/pagination. Сравните popular/search API projection: optional stable tag `code` должен быть одинаковым. Запрос с noncanonical host/port/scheme обязан получить `X-Seasonvar-Page-Cache: BYPASS`, а canonical HTTPS — только canonical asset/meta URLs. Authenticated controlled account проверяет private CRUD/restore/batch Apply/Cancel/library filter/export и отсутствие автоматической подстановки interface locale; admin — отдельные `ru|en` translation/alias rows, stale-version rejection, synonym/archive/restore/provider decision/global assignment/merge preview. Guest HTML/cache/sitemap/API не должен содержать private UUID/label/count.
...
## Rollout заявок на материалы

1. Убедитесь, что existing import/finalizer/title merge terminal, сделайте verified SQLite backup и foreign-key/integrity preflight. Task 19 migration additive и не требует data cleanup, потому что audit не нашёл legacy request/ticket tables; destructive commands не нужны.
2. Deploy rolling-safe code с `ContentRequestSchema`, затем `php artisan migrate --force`. Сверьте восемь новых таблиц, public UUID/active/submission uniqueness, vote/follow uniqueness, target/status/title/external/history indexes и FK `nullOnDelete`/engagement cascade contract.
3. Rebuild config/routes/views and Vite assets, затем graceful-reload PHP-FPM и existing workers. Текущий production route cache должен быть пересобран обычным deploy workflow, иначе новые `/requests`, localized/admin и `sitemap-requests-*` routes не появятся в running process.
4. Smoke guest directory/detail/noindex pre-review, verified create/search/existing-content/duplicate, vote/follow, My Requests/settings/inbox; moderator clarification/reject/merge/priority/partial/full/handoff; old merged UUID redirect; sitemap eligibility только approved/planned/in-progress/partial/completed.
...
1. Deploy rolling-safe code, then apply pending additive migrations only inside the common stopped-writer/backup SQLite boundary: `2026_07_16_120000_add_canonical_recommendation_discovery.php`, `2026_07_16_220000_add_recommendation_release_event_index.php`, `2026_07_16_230000_add_recommendation_signal_timestamps.php`, `2026_07_16_230100_add_recommendation_signal_owner_indexes.php`, `2026_07_16_240000_create_catalog_recommendation_shadow_builds.php` and `2026_07_16_240100_create_catalog_recommendation_dirty_titles.php` as applicable. Verify semantic fields/indexes plus shadow build/source-rank/candidate-score indexes and unique dirty title queue. Migrations `240000/240100` do not rewrite or delete active similarity. Never rebuild/wipe existing similarity or user data.
2. Rebuild config/routes/views and Vite assets, then graceful-reload PHP-FPM/workers. Production route cache must include `/discover/{type}`, localized routes and legacy redirects. Ranking key version is `task18-v6-r2`, so pre-hardening `v5` pools and the former `indexed_at`-ordered `recently_added` pool become unreachable without a global cache scan or flush.
3. Synchronous maintenance-запуск существующей команды `seasonvar:import` строит similarity v6 в shadow. Queued Seasonvar finalizer выполняет только bounded scoped rebuild: при отсутствии active v6, version mismatch или превышении source/dirty limit он сохраняет dirty queue, записывает `deferred` и завершает импорт, не повторяя catalog-wide build в каждом 900-секундном job. Existing active rows remain serviceable until the golden gate passes; do not delete them manually. Record build ID, baseline/candidate metrics, judged coverage, empty sources, availability and churn. Only a non-empty `active` v6 build may replace active rows; `allow_activation_without_golden` не разрешает пустую замену. Active build сохраняется независимо от retention; после обычного завершения или аварии активации автоматически остаются только последние `recommendations.similarity_v6.build_history_limit` terminal build'ов (по умолчанию 5), а их старые owned shadow rows удаляются каскадно. Profile/scoring chunks обновляют build heartbeat; `building` без heartbeat дольше `recommendations.similarity_v6.stale_build_minutes` (по умолчанию 20 минут) автоматически становится `failed`, не меняя active rows. Collection-sync job использует тот же bounded контракт и не запускает cache warm при defer. Ошибка cleanup только регистрируется и не маскирует исходный результат или сбой build. После первой activation bounded signal pruner удаляет только legacy generic `seasonvar_info` signals; provider/related rows должны сохраниться.
4. Personalization v2 после deploy остаётся на `RECOMMENDATIONS_PERSONALIZED_V2_ENABLED=false` и `RECOMMENDATIONS_PERSONALIZED_V2_PERCENT=0`. Не повышайте процент, пока нет active `v6` build с валидными `score_min`, `score_median`, `score_p95`. После `config:cache` и reload выполняйте этапы `0 → internal fixture → 10 → 50 → 100`, проверяя 0 exact-exclusion violations, 100% watchability, 0 explore-floor violations, отсутствие private IDs/labels/progress в URL/HTML/Livewire/API/shared cache, fallback share, repeats, faithful reasons и errors. Любое нарушение возвращает процент к `0`; content v6 не откатывается.
5. Smoke anonymous public types/filters/random, authenticated cold/low/medium/high, personal feedback/undo/library restore, deterministic refresh/exploration, title related-before-similar, homepage/search, public API compatibility and admin relation authorization. Verify personal/random/filter pages noindex and only nonempty stable public types in sitemap.
6. Rollback web code may leave additive rows inert. `240100` removes only pending dirty state; `240000` removes only shadow history and never active `catalog_title_recommendations`. Before any schema down after real feedback/editorial writes, export/backup them; task 18 downs do not remove watchlist/rating/progress/stored active similarity/editorial collections. Prefer roll-forward because removing state would lose explicit user feedback/provenance and build evidence.
...
2. Проверьте production `PLAYBACK_ALLOWED_HOSTS`, `PLAYBACK_ENFORCE_PUBLIC_DNS=true` и `SEASONVAR_MEDIA_FILE_SIZE_*`; новый download всё равно принудительно требует public DNS и standard HTTPS независимо от playback compatibility switch. Затем `config:cache`, `route:cache`, PHP-FPM reload и worker restart должны загрузить новые config/casts/route/policy.
3. Убедитесь, что named route `titles.media.download` имеет `auth`, `auth.session`, `account.private`, `throttle:media-downloads` и scoped title/media binding. Shared proxy/CDN не должен переопределять `private, no-store` или кешировать attachment body.
4. Запустите обычный importer: new/changed direct media проверяются автоматически, terminal counters видны в CLI/admin. Scheduler постепенно обрабатывает legacy backlog через `seasonvar:import --refresh-media-sizes --media-size-limit=500 --media-size-time-budget=480` каждые 10 минут под scheduler/import single-flight. `SEASONVAR_MEDIA_FILE_SIZE_SCHEDULED_BACKFILL_LIMIT` задаёт hard count cap, `SEASONVAR_MEDIA_FILE_SIZE_SCHEDULED_BACKFILL_TIME_BUDGET_SECONDS` — budget 1–3600 секунд; после изменения пересоберите config cache и перезапустите scheduler/workers. Для временной паузы задайте `SEASONVAR_MEDIA_FILE_SIZE_SCHEDULED_BACKFILL_ENABLED=false`. Ручной bounded запуск использует тот же path; `--force-media-sizes` оставьте только для контролируемой повторной проверки. Немедленный полный catalogue backfill не требуется.
5. Smoke authenticated direct download должен подтвердить filename, 200 и one valid 206 resume; guest/other-title/unpublished/HLS/malformed range — отсутствие bytes. Никогда не сохраняйте body для smoke в repository/application storage; используйте headers и bounded client cancellation.
6. Планируйте upstream и исходящий bandwidth, длительность PHP-FPM worker и provider limits. Defaults ограничивают inspection count/timeouts/retries, а download stream timeout/chunk/request rate находятся в `playback.downloads`; HLS aggregation/transcoding/offline copy отсутствуют.

## Rollout технических обращений Task 20

1. Deploy rolling-safe code и примените additive `2026_07_16_200000_create_technical_issue_domain.php`, `2026_07_16_200100_add_requester_order_index_to_technical_issues.php`, затем `2026_07_17_141321_add_sort_ranks_to_technical_issues_table.php`; первая migration создаёт только 13 новых таблиц, вторая — covering index requester-list, третья — два derived rank column, безопасный backfill stable code и два индекса staff sorting. До полного DDL `TechnicalIssueSchema` показывает безопасный unavailable state. Не редактируйте старые migrations и не выполняйте `migrate:fresh`/`db:wipe`.
2. Rebuild config/routes/views и Vite assets штатным workflow, reload PHP-FPM. Проверьте canonical/localized `issues.*`, staff `admin.issues`, private attachment middleware, `manage-technical-issues` gate и `livewire-uploads` limiter. Shared proxy не должен кешировать private/no-store responses.
...
## Rollout синхронизации дат серий Seasonvar с календарём

Изменение не добавляет migration, dependency, environment variable, новую очередь или публичную команду. После обычного выпуска PHP-кода выполнить graceful reload PHP-FPM и `php artisan queue:restart`, чтобы долгоживущие import workers получили актуальный synchronizer. Не выполнять `cache:clear`, `queue:clear` или массовый retry.

Перед one-time backfill проверить `php artisan seasonvar:import --status`, `php artisan app:deployment-check --json` и `php artisan migrate:status`: предыдущий global run должен быть terminal, а живые claims и конкурирующие SQLite writers — отсутствовать либо безопасно завершиться штатно. Поскольку schema не меняется, отдельный migration backup не требуется, но действующая production backup policy и проверка свободного места остаются обязательными для 1000-page writer run.

Запуск выполняется только существующей границей:

```bash
php artisan seasonvar:import --queued --force --sitemap-tail=1000
```

Наблюдение выполняется через `php artisan seasonvar:import --status` до terminal state. Watchdog/finalizers остаются штатным recovery path; interrupted запуск не восстанавливается очисткой очереди или claims. Post-deploy проверка подтверждает на контрольных тайтлах provider calendar entry для точной серии/перевода, отсутствие записи provider-факта в `episodes.released_at`, одну logical row и видимость события на `/calendar`.
