...
1. Deploy rolling-safe code, then apply pending additive migrations only inside the common stopped-writer/backup SQLite boundary: `2026_07_16_120000_add_canonical_recommendation_discovery.php`, `2026_07_16_220000_add_recommendation_release_event_index.php` and `2026_07_16_230000_add_recommendation_signal_timestamps.php` as applicable. Verify the relation table, feedback/status fields, nullable `watchlist_updated_at|rating_updated_at|watch_status_updated_at`, `episodes_recommendation_release_events_idx` and both semantic user-state activity indexes. Migration `230000` deliberately performs no data backfill: do not copy shared `updated_at` into semantic columns manually. Never rebuild/wipe existing similarity or user data.
2. Rebuild config/routes/views and Vite assets, then graceful-reload PHP-FPM/workers. Production route cache must include `/discover/{type}`, localized routes and legacy redirects. Ranking key version is `task18-v5`, so no global cache flush is required.
