...
4. Запустите обычный importer: new/changed direct media проверяются автоматически, terminal counters видны в CLI/admin. Backfill запускайте отдельно небольшими операционными пакетами: `seasonvar:import --refresh-media-sizes --media-size-limit=N`; `--force-media-sizes` только для контролируемой повторной проверки. Немедленный полный catalogue backfill не требуется.
