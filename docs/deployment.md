...
2. Rebuild config/routes/views and Vite assets, then graceful-reload PHP-FPM/workers. Production route cache must include `/discover/{type}`, localized routes and legacy redirects. Ranking key version is `task18-v6`, so pre-hardening `v5` pools become unreachable without a global cache scan or flush.
