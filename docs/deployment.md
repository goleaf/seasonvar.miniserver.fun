...
Повторное усиление 18.07.2026 добавляет только idempotent data repair `2026_07_18_220000_repair_removed_review_deletion_state.php`; на configured production-style SQLite оно остаётся `Pending` и этой задачей не применяется. Перед rollout остановить review/admin/demo/title-merge writers, сделать verified backup и записать exact set `status=removed` с отсутствующими `deletion_reason|deleted_by_id|deleted_at`. После `php artisan migrate --force` проверить, что изменились только эти nullable evidence fields, body/ID/title/ownership/vote/report/alias counts совпадают, `PRAGMA quick_check` и foreign keys чисты, а `/reviews/{id}` и `/{ru|en}/reviews/{id}` используют один responder. `down()` не обнуляет восстановленное evidence; rollback выполняется возвратом совместимого code или forward fix, не data corruption. Global cache flush и новый worker не требуются.
...
3. The existing full Seasonvar import builds similarity v6 in shadow. Existing active rows remain serviceable until the golden gate passes; do not delete them manually. Record build ID, baseline/candidate metrics, judged coverage, empty sources, availability and churn. Only a non-empty `active` v6 build may replace active rows; `allow_activation_without_golden` не разрешает пустую замену. Active build сохраняется независимо от retention; после обычного завершения или аварии активации автоматически остаются только последние `recommendations.similarity_v6.build_history_limit` terminal build'ов (по умолчанию 5), а их старые owned shadow rows удаляются каскадно. Profile/scoring chunks обновляют build heartbeat; `building` без heartbeat дольше `recommendations.similarity_v6.stale_build_minutes` (по умолчанию 20 минут) автоматически становится `failed`, не меняя active rows. Collection-sync job выполняет только bounded scoped rebuild: при version mismatch или превышении source/dirty limit она сохраняет dirty queue, не создаёт full build и не запускает cache warm; полный fallback принадлежит следующему штатному `seasonvar:import`. Ошибка самого cleanup только регистрируется и не маскирует исходный результат или сбой build. После первой activation bounded signal pruner удаляет только legacy generic `seasonvar_info` signals; provider/related rows должны сохраниться.
