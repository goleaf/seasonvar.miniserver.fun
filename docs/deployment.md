...
4. Запустите обычный importer: new/changed direct media проверяются автоматически, terminal counters видны в CLI/admin. Scheduler постепенно обрабатывает legacy backlog через `seasonvar:import --refresh-media-sizes --media-size-limit=N` (default 20 каждые 10 минут) под scheduler/import single-flight; для временной паузы задайте `SEASONVAR_MEDIA_FILE_SIZE_SCHEDULED_BACKFILL_ENABLED=false` и перестройте config cache. Ручной bounded запуск использует тот же path; `--force-media-sizes` оставьте только для контролируемой повторной проверки. Немедленный полный catalogue backfill не требуется.
