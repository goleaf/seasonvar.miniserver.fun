...
Operational rollback начинается с `HDREZKA_COLLECTION_SYNC_ENABLED=false`, config-cache rebuild и graceful worker reload: существующие локальные подборки продолжают открываться, а новые runs не стартуют. После production writes сохраняйте source audit, collections и private WebP в verified backup; migration rollback удалит audit и exact-match индексы, поэтому выполняется только вместе с совместимым code rollback. Не используйте `cache:clear`, `queue:clear`, `migrate:fresh`, `db:wipe` или удаление uploads tree.
