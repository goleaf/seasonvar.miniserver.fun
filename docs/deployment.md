...
`demo:repair-user-portal` не заменяет production seeding и не принимает произвольных пользователей. Сначала выполнить только `php artisan demo:repair-user-portal --dry-run --json`, проверить точное число известных demo accounts, свободное место, очередь `cache-warm-v2`, отсутствие активного импорта и незавершённой recommendation build, текущие invalid image/request counters, `legacy_demo_tag_expected_pairs`, `legacy_demo_tag_matched_pairs`, `legacy_demo_owned_tags`, `legacy_demo_match_basis_points`, `orphaned_demo_public_tag_assignments` и `archivable_demo_public_tags`. Historical tag repair разрешён только при exact versioned `code` + deterministic UUIDv5 `public_id` fingerprint и достаточной плотности прежних назначений; mutable имя/slug не считаются ownership evidence. Перед записью остановить web/CLI/queue writers штатными средствами реально установленной инфраструктуры, создать согласованный SQLite backup вне публичного и tracked storage, проверить backup через `PRAGMA quick_check`, foreign keys, размер и контрольную сумму. Только затем разрешён `php artisan demo:repair-user-portal --force --backup-confirmed --writers-paused --json`.

После repair нужно повторить dry-run: `users_without_library`, `users_without_personal_tags`, `users_without_requests`, `invalid_profile_images`, `orphaned_demo_public_tag_assignments` и `archivable_demo_public_tags` должны быть равны нулю; collection-image counter отсутствует, потому что подборки не имеют image domain. Для тегов отдельно проверить известный title, отсутствие шумных aggregate pivots, сохранение current provider/editorial provenance, dirty recommendation markers и отсутствие stale active recommendation rows. Затем запустить writers, проверить worker `cache-warm-v2`, открыть `/requests`, `/library`, `/library/tags/manage`, `/profile`, tag/search/title pages и collection section `/discover/popular` под demo account, убедиться в отсутствии `404` у versioned profile media и в text-only карточках подборок. При partial failure не выполнять `cache:clear`, `queue:clear` или повторный полный seed: команда атомарна и идемпотентно повторяется только после диагностики, terminal writers и нового backup. Data rollback — восстановление проверенной SQLite backup при всё ещё остановленных writers; после появления новых пользовательских записей предпочтителен roll-forward, чтобы их не потерять.
