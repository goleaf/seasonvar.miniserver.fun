...
1. Deploy rolling-safe code, then apply pending additive migrations only inside the common stopped-writer/backup SQLite boundary: `2026_07_16_120000_add_canonical_recommendation_discovery.php`, `2026_07_16_220000_add_recommendation_release_event_index.php`, `2026_07_16_230000_add_recommendation_signal_timestamps.php` and `2026_07_16_230100_add_recommendation_signal_owner_indexes.php` as applicable. Verify the relation table, feedback/status fields, nullable `watchlist_updated_at|rating_updated_at|watch_status_updated_at`, `episodes_recommendation_release_events_idx`, both public semantic activity indexes and the three owner-first personalized-order indexes. Migration `230000` deliberately performs no data backfill: do not copy shared `updated_at` into semantic columns manually. Migration `230100` adds indexes only and rolls them back independently. Never rebuild/wipe existing similarity or user data.
...
3. The existing full Seasonvar import performs similarity v5 rebuild. Existing v4 rows remain serviceable until that run; do not delete them manually. Verify algorithm counts, zero self/hidden targets and public cache version after terminal rebuild.
4. Smoke anonymous public types/filters/random, authenticated cold start/personal feedback/undo/library restore, title related-before-similar, homepage/search, public API compatibility and admin relation authorization. Verify personal/random/filter pages noindex and only nonempty stable public types in sitemap.
5. Rollback web code may leave additive rows inert. Before any schema down after real feedback/editorial writes, export/backup them; down intentionally removes only task 18 relation/state columns/indexes, not watchlist/rating/progress/stored similarity/editorial collections. `230000` first restores the exact technical-timestamp legacy index definitions and then removes its three semantic columns. Prefer roll-forward because removing state would lose explicit user feedback/provenance.
