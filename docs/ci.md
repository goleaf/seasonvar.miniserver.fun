...
`pre-commit` после проверок чистоты дерева сначала запускает `scripts/update-changelog-for-staged-code.sh`, а затем тот же профиль `docs` и политики README/CHANGELOG. При наличии подготовленных изменений кода и отсутствии ручной записи updater автоматически добавляет краткую русскую датированную запись и выполняет точный `git add -- CHANGELOG.md`; это единственная разрешённая мутация индекса. Поэтому stale managed blocks по-прежнему блокируются до создания commit. Они исправляются явно командой `php artisan project:docs-refresh`, после проверки изменённых управляемых документов gate запускается повторно; остальные файлы hook не исправляет и не добавляет в индекс.
