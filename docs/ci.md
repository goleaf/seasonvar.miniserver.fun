...
`composer git:doctor` является отдельной read-only диагностикой transport и локального Git-состояния, а не quality gate и не заменой `scripts/ci-check.sh`, hooks или GitHub Actions. По умолчанию doctor не использует сеть, принимает только canonical HTTPS/SSH URL этого repository и для SSH требует repository-local explicit identity с `IdentitiesOnly=yes` и `BatchMode=yes`, не выводя remote, команду или путь к ключу. Режим `composer git:doctor -- --remote` выполняет только bounded `git ls-remote`, не изменяет refs и не доказывает write access. Единственным локальным publish gate остаётся versioned `pre-push`: он требует clean tree и запускает полный профиль `pre-push` до обычного `git push origin main`.
...
`pre-commit` после проверок ветки, конфликтов и безопасных staged-путей сначала запускает `scripts/update-changelog-for-staged-code.sh`, а затем тот же профиль `docs` и staged-политики README/CHANGELOG. При наличии подготовленных изменений кода и отсутствии ручной записи updater автоматически добавляет краткую русскую датированную запись и выполняет точный `git add -- CHANGELOG.md`; это единственная разрешённая мутация индекса. Посторонние unstaged/untracked файлы не блокируют partial commit и не добавляются hook. Профиль `docs` даёт раннюю проверку текущего рабочего снимка; окончательная проверка целостного snapshot выполняется `pre-push` при обязательном clean tree. Устаревшие управляемые блоки исправляются явно командой `php artisan project:docs-refresh`, после проверки изменённых управляемых документов gate запускается повторно; остальные файлы hook не исправляет и не добавляет в индекс.
