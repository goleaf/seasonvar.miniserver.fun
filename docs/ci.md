...
`composer git:doctor` является отдельной read-only диагностикой transport и локального Git-состояния, а не quality gate и не заменой `scripts/ci-check.sh`, hooks или GitHub Actions. По умолчанию doctor не использует сеть; режим `composer git:doctor -- --remote` выполняет только bounded `git ls-remote`, не изменяет refs и не доказывает write access. Единственным локальным publish gate остаётся versioned `pre-push`: он требует clean tree и запускает полный профиль `pre-push` до обычного `git push origin main`.
