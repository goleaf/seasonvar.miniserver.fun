...
Task 14 does not invent follower/profile-activity notifications because no follow/activity domain exists. Profile reports remain private moderation rows and do not reuse comment/review message payloads. Existing comment/review/collection notifications now may link the same public author profile only through their prepared presenters; their category preferences, self-suppression, blocks/mutes, spoiler rules and body-free security boundaries remain unchanged.
...
## Уведомления технических обращений

`TechnicalIssueActivityNotification` использует existing database channel и deterministic UUID recipient+ticket+revision+category+canonical. Categories: submitted, clarification, support reply, status changed, assigned, resolved, resolution verified, closed, reopened, rejected, merged. Public updates ограничены requester/followers/confirmers; assignment — private notification только новому assignee, без requester routing noise. Actor self-notification и duplicate retry подавляются.

`technical_issue_notification_preferences` независимо управляет requester/confirmer/follower/support-reply updates и не меняет Task 16 comment/review/request keys. Payload содержит stable ticket UUID/number/type/status/category/revision/canonical link и не содержит user text, attachment, diagnostics, source URL, IP/UA, staff identity, internal note или recipient list. Inbox повторно применяет ticket policy. Полный matrix: [`technical-issues.md`](technical-issues.md).
