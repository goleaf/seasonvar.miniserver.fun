...
Blade получает DTO/options/paginators и не вызывает models/services/config/database, не рассчитывает duplicate/priority/transition/permissions и не сериализует Eloquent graph. Все mutations имеют targeted `wire:loading`, disabled retry protection, status/alert live regions и explicit confirmation для withdrawal/merge/import. Layout использует существующие light-theme panels/forms/badges/buttons/focus classes; grids collapse на phone, long title/user text/URL uses break rules, actions wrap without hover-only access. Нового Vite module не потребовалось: request business logic и trusted validation не перенесены в JavaScript.
...
Под категорийной навигацией находится одна пассивная GET-форма с native number inputs и country select: на телефоне поля идут одной колонкой, на среднем экране — двумя, на широком — одной строкой. Submit работает без JavaScript, выбранные значения сохраняются в category links, reset появляется только при активных условиях, validation errors связаны с controls через `aria-describedby`, а filtered empty state предлагает сброс текущей категории.
