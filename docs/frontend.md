...
Контекстная ссылка «Исправить данные» является progressive обычным `<a>` с `data-correction-field`, видимым focus и высотой не менее 44 px. Она присутствует у 11 поддержанных полей, а episode/subtitle controls вынесены за пределы `wire:ignore` и соседних episode anchors. Browser back/forward и guest login redirect сохраняются стандартными route/query contracts; отдельный JavaScript/modal lifecycle не добавлен.
