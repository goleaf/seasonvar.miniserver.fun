...
- Header search остаётся видимым на всех публичных routes, включая `/titles`, `/titles/year/{year}` и taxonomy listing pages. Локальная поисковая форма каталога находится над результатами и имеет отдельное доступное имя `Поиск по каталогу` или `Искать в выбранной подборке`, поэтому на listing routes допустимы два разных search landmarks без дублирования input IDs. Мобильные фильтры открываются через native `<dialog>`; desktop использует тот же DOM-узел как sticky sidebar. До `loadFacets()` этот узел содержит только пояснение и кнопку, поэтому сотни скрытых options не входят в initial HTML/Livewire payload. Mobile trigger одновременно открывает dialog и запускает загрузку; Native Escape закрывает dialog, обработчик `close` возвращает focus trigger.
- Серверное состояние `/titles` ведёт `CatalogSeries`: строка поиска использует отложенный `wire:model` и применяется по submit, checkbox/расширенные поля применяются по submit, а сортировка, вид, размер страницы, алфавит и пагинация обновляются отдельными Livewire actions. Новый поиск сбрасывает `facetsLoaded`, чтобы карточки возвращались раньше contextual facets. Для всех форм сохранён обычный GET fallback; malformed и out-of-range `page` канонизируется redirect-ом, чтобы адресная строка не сохраняла stale границу.
