...
- Главная, каталог и рекомендации получают личные признаки карточек через bounded eager loader до Blade render: watchlist, personal rating, progress и primary action не выполняют запросы из `x-catalog.title-card` и не попадают в guest shared cache. Публичная карточка дополнительно получает только два возможных provider rating rows (`kinopoisk`, `imdb`) и выбирает КиноПоиск с fallback на IMDb без lazy loading.
...
- Все каталожные изображения выводит `x-ui.poster-frame` с явным `fit`. Контентные строки используют `2:3`, `object-contain` и отключённый overscan, поэтому весь постер виден на телефоне, планшете и desktop; cover остаётся только у главного постера и технических миниатюр, где он запрошен явно. `x-ui.poster-card` поддерживает `list`, `compact`, `recommendation`, `stats`; обычные коллекции `CatalogTitle` используют query-free `x-catalog.title-card`. Общая карточка отдаёт только server-side excerpt до 240 Unicode-символов с `line-clamp-3` как visual guard, максимум три жанра, год, provider rating, число серий, явную ссылку «Подробнее» и для recommendation layout одну broad-причину. Полное описание не сериализуется скрытым текстом карточки.
