...
- Единственная публичная directory-точка рекомендаций и подборок — `/discover/{type}`; каждый поддерживаемый discovery mode содержит одну и ту же независимую секцию публичных коллекций с query keys `collections_q`, `collections_sort`, `collections_category`, `collections_subcategory` и `collectionsPage`. Совместимый вход `/discover` и его localized aliases выполняют временный redirect на соответствующий `popular#collections`, не создавая второй directory owner. Секция сохраняет `#collections`, располагается перед выдачей сериалов и не меняет mode-specific ranking или filters.
...
- Preview/selection/confidence не являются authority. Подтверждение повторно требует `content.manage`, authoritative active category/root, exact UUID и совпавший `content_version`; один пакет ограничен 100 строками, а material assignment использует существующие audit и after-commit collection invalidation boundaries.
- Удалённые `/collections`, `/admin/collections`, `/recommendations`, `/lists`, `/selections` и `/my/lists` возвращают `404` без compatibility redirects по явному продуктовому решению. Единственное исключение — `/discover` и localized aliases: они остаются совместимыми entry routes к каноническому `popular#collections`; новые aliases для остальных удалённых адресов не добавляются.
