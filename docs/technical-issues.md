...
Не собираются raw user-agent, IP, headers, cookies, session ID, bearer/CSRF/reset/verification/OAuth/provider tokens, local/session storage, arbitrary form data/history, clipboard, password/MFA/payment data, precise location, private export/reset URLs, protected media/source URL или exception/SQL/stack/storage path. Нет client fingerprint, определения провайдера/полосы пропускания и фонового speed test. Raw UA/IP остаются только в уже существующих auth/security boundaries и не копируются в тикет.

Исключение для явной кнопки «Видео не работает» ограничено безопасным anonymous network probe: только после клика браузер один раз запрашивает фиксированный same-origin endpoint без пользовательских URL или payload, измеряет bounded round-trip и передаёт allowlisted статус `ok|failed|offline|timeout` и целое время задержки. Этот probe не читает Network Information API, не определяет IP, оператора, географию или скорость канала, не запускается в фоне и не повторяется при ошибке. Валидный server-issued playback report token может заранее включить diagnostic consent на форме, но checkbox остаётся видимым и отзывным до отправки. Без согласия playback snapshot в тикет не копируется.

Обезличенная first-party playback quality telemetry хранит отдельный случайный request ID, канонические IDs title/season/episode/media, server-derived provider/variant/quality/translation/format, allowlisted browser/OS/HLS capability, стабильную категорию ошибки, startup/playback/buffering durations, количество buffering, признаки primary/fallback failure и fallback outcome. Она не содержит `user_id`, IP, raw UA, session/cookie/token, source/grant URL или storage path, не изменяет source health автоматически и очищается bounded retention-командой. Тикет по согласию получает самостоятельный snapshot нужных полей, поэтому staff evidence не зависит от срока жизни агрегированной telemetry.
...
Операционные ticket/history сохраняются. Команда `php artisan technical-issues:prune-private-data --limit=200` независимо и bounded batches удаляет anonymous playback sessions старше `playback.quality.retention_days` (по умолчанию 90 дней), optional diagnostics terminal tickets после 180 дней, redacts occurrence diagnostics и удаляет screenshots только у closed/withdrawn tickets старше 365 дней. Согласованный playback snapshot внутри обращения следует retention самого ticket diagnostic, поэтому остаётся понятным после удаления anonymous session. Cron/queue не обязательны и автоматически не добавлены; operator запускает command согласно privacy runbook. Rejected/merged evidence сохраняется для moderation/audit до отдельного утверждённого retention policy.
...
Task 87 добавляет expiring playback report token поверх прежнего context, не заменяя его. Server повторно связывает telemetry request с текущим title/media и после явного consent копирует в `technical_issue_diagnostics` только request UUID, server-owned variant/quality/translation/format/provider, сокращённые browser/OS/HLS codes, error category/source, startup/playback/buffering counters, primary/fallback outcome и fixed same-origin network round-trip. Перед submit форма показывает этот точный snapshot и позволяет снять consent; тогда все новые diagnostic columns остаются `null`. Raw UA/IP/URL/grant/cookie/token/provider response/stack trace не копируются ни при каком состоянии client payload.
