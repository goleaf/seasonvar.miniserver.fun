...
- Recommendation rebuild не материализует вложенные Eloquent-профили и не перебирает весь каталог для каждого тайтла. Профили сериализованы компактно, singleton features удаляются из packed inverted index, candidate scan ограничен на один признак и на один тайтл до exact scoring. На рабочем каталоге из 34 319 публичных тайтлов контрольный read-only профиль под `memory_limit=128M` показал peak 116,5 МБ, около 53 секунд на index и оценочно 93 секунды на scoring; эти цифры являются эксплуатационным baseline, а не гарантией для каталога другого размера.
