...
- Завершённые local/media/cleanup/merge стадии queued finalization сохраняют bounded versioned checkpoint в summary run. Hard timeout не заставляет следующую доставку повторно сканировать каталог, а invalid/неполная версия безопасно заставляет повторить authoritative стадии. Queued recommendation handoff выполняет только bounded scoped/noop rebuild: если нужен catalog-wide full shadow build, он возвращает `deferred`, сохраняет dirty IDs и прежние active rows до контролируемого synchronous maintenance-запуска, а текущий run завершается без повтора 900-секундных job. Metadata eligibility дополнительно ограничена вычисляемыми `format`/`variant_type`/`variant_key`; optional `quality`/`translation_name`, которые детерминированно определить нельзя, не создают постоянный backlog.
