...
- `CatalogSeries::render()` вызывает `CatalogTitlesPageBuilder` один раз; `mount()` только валидирует URL и не читает каталог. Контрольный guest-запрос `/titles` выполняет 21 SQL-запрос с учётом трёх session-запросов, как и до Livewire-рефакторинга.
- Initial Livewire snapshot каталога содержит только `filters`, три locked route-поля и `paginators` (около 1,1 КБ на контрольной базе); paginator, модели и facet-коллекции остаются только в render data.
