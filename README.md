...
- Livewire 4.3 используется для live-страницы статистики `/stats`.
...
Полная локальная установка, MCP и правила разработки описаны в `docs/development.md` и `docs/mcp.md`.
...
Страница `/stats` построена на Livewire 4, обновляет все видимые блоки раз в секунду через `wire:poll.1s` и читает подготовленный snapshot из cache, чтобы не пересобирать тяжелые агрегаты на каждый browser poll.
