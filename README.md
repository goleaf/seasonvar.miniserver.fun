...
Каждый отрендеренный источник получает отдельную browser-session Plyr/HLS. Она сохраняет bounded progress раз в 30 секунд только во время просмотра, фиксирует позицию при паузе, стабильном завершении перемотки, скрытии вкладки и уходе со страницы, отклоняет события от уже заменённой серии и освобождает HLS/Plyr/listeners/timers при Livewire morph, `wire:navigate`, `pagehide` или возврате страницы из back-forward cache. Маркеры очищаются и с восстановленного Plyr media node, поэтому повторная навигация не создаёт дубликаты плеера. Ошибки и автоматические повторы показываются локальными русскими состояниями без raw provider URL.
