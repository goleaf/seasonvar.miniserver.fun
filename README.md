...
Десять relation-фасетов `/titles` выполняются одним bounded `UNION ALL`, причем limit применяется внутри каждой группы. Карточки и title summaries выбирают только отображаемые поля связей. Каталог и приватные progress/history данные не помещаются в shared cache; кешируется только обезличенный агрегированный snapshot `/stats`, который точечно обновляется после importer finalization.
...
- Обновлено командой `php artisan project:docs-refresh`: 13.07.2026.
