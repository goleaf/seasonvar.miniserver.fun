<div class="mx-auto max-w-5xl space-y-5">
    ...
    @island(name: 'profile-discussion-pagination', always: true, with: $this->paginationIslandPage)
    @if (! $available)
    ...
    @else
        <x-ui.pagination-region name="profile-discussion-activity-results">
        <section aria-labelledby="discussion-activity-title" class="rounded-panel border border-slate-200 bg-white shadow-panel">
            ...
            @else
                ...
                @if ($activity->hasPages())
                    <nav class="p-4" aria-label="{{ __('comments.accessibility.pagination') }}">{{ $activity->links(data: ['region' => 'profile-discussion-activity-results']) }}</nav>
                @endif
            @endif
        </section>
        </x-ui.pagination-region>

        <x-ui.pagination-region name="profile-comment-notifications-results">
        <section aria-labelledby="discussion-notifications-title" class="rounded-panel border border-slate-200 bg-white shadow-panel">
            ...
            @else
                ...
                @if ($notifications->hasPages())
                    <nav class="p-4" aria-label="{{ __('comments.notifications.title') }}">{{ $notifications->links(data: ['region' => 'profile-comment-notifications-results']) }}</nav>
                @endif
            @endif
        </section>
        </x-ui.pagination-region>

        <x-ui.pagination-region name="profile-review-notifications-results">
        <section aria-labelledby="review-notifications-title" class="rounded-panel border border-slate-200 bg-white shadow-panel">
            ...
            @else
                ...
                @if ($reviewNotifications->hasPages())
                    <nav class="p-4" aria-label="{{ __('reviews.notifications.title') }}">{{ $reviewNotifications->links(data: ['region' => 'profile-review-notifications-results']) }}</nav>
                @endif
            @endif
        </section>
        </x-ui.pagination-region>

        <x-ui.pagination-region name="profile-request-notifications-results">
        <section aria-labelledby="request-notifications-title" class="rounded-panel border border-slate-200 bg-white shadow-panel">
            ...
            @else
                ...
                @if ($requestNotifications->hasPages())<nav class="p-4" aria-label="{{ __('requests.notifications.title') }}">{{ $requestNotifications->links(data: ['region' => 'profile-request-notifications-results']) }}</nav>@endif
            @endif
        </section>
        </x-ui.pagination-region>
        ...
        <section aria-labelledby="discussion-privacy-title" class="rounded-panel border border-slate-200 bg-white p-4 shadow-panel sm:p-5">
            ...
            <div class="mt-4 grid gap-4 lg:grid-cols-2">
                <x-ui.pagination-region name="profile-blocks-results">
                <div>
                    ...
                    @else
                        ...
                        @if ($blocks->hasPages())
                            <nav class="mt-3" aria-label="{{ __('comments.profile.blocks') }}">{{ $blocks->links(data: ['region' => 'profile-blocks-results']) }}</nav>
                        @endif
                    @endif
                </div>
                </x-ui.pagination-region>
                <x-ui.pagination-region name="profile-mutes-results">
                <div>
                    ...
                    @else
                        ...
                        @if ($mutes->hasPages())
                            <nav class="mt-3" aria-label="{{ __('comments.profile.mutes') }}">{{ $mutes->links(data: ['region' => 'profile-mutes-results']) }}</nav>
                        @endif
                    @endif
                </div>
                </x-ui.pagination-region>
