                ...
                <div class="space-y-3 rounded-panel border border-slate-300 bg-slate-50 p-3 sm:p-4">
                            ...
                            <div class="mt-3 space-y-3">
                                @foreach ($adminItem->reports as $report)
                                    <article wire:key="review-report-{{ $report['id'] }}" class="rounded-control bg-white p-3 text-sm text-slate-700">
                                        ...
                                        @if (in_array($report['status'], ['open', 'reviewed'], true))
                                            <label class="mt-3 block text-sm font-bold text-slate-700">{{ __('reviews.moderation.private_note') }}
                                                <textarea wire:model="reportNotes.{{ $report['id'] }}" maxlength="2000" rows="2" class="mt-1 w-full rounded-control border border-slate-300 bg-white px-3 py-2.5"></textarea>
                                                <span class="mt-1 block text-xs font-normal text-slate-500">{{ __('reviews.moderation.private_note_hint') }}</span>
                                            </label>
                                            <div class="mt-3 flex flex-wrap gap-2">
                                                <button type="button" wire:click="resolveReport({{ $report['id'] }}, 'resolved')" wire:confirm="{{ __('reviews.reports.resolve_confirm') }}" wire:loading.attr="disabled" wire:target="resolveReport({{ $report['id'] }}, 'resolved')" class="inline-flex min-h-11 items-center rounded-control bg-emerald-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-60">{{ __('reviews.reports.statuses.resolved') }}</button>
                                                <button type="button" wire:click="resolveReport({{ $report['id'] }}, 'dismissed')" wire:confirm="{{ __('reviews.reports.dismiss_confirm') }}" wire:loading.attr="disabled" wire:target="resolveReport({{ $report['id'] }}, 'dismissed')" class="inline-flex min-h-11 items-center rounded-control bg-slate-200 px-3 py-2 text-xs font-bold text-slate-700 disabled:opacity-60">{{ __('reviews.reports.statuses.dismissed') }}</button>
                                            </div>
                                        @endif
                                    </article>
                                @endforeach
                    ...
                    <form wire:submit="moderateReview({{ $adminItem->review->id }})" class="grid gap-3 rounded-control border border-slate-200 bg-white p-4 lg:grid-cols-2">
                        <label class="text-sm font-bold text-slate-700">{{ __('reviews.moderation.status') }}
                            <select wire:model="moderationStatuses.{{ $adminItem->review->id }}" class="mt-1 min-h-11 w-full rounded-control border border-slate-300 bg-white px-3 py-2.5">@foreach ($reviewStatusOptions as $option)<option value="{{ $option['value'] }}">{{ $option['label'] }}</option>@endforeach</select>
                        </label>
                        ...
                        @if ($adminItem->activeRestriction)
                            <div class="flex flex-col gap-3 rounded-control border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    @if ($adminItem->activeRestriction['private_note'])
                                        <p class="mt-2 whitespace-pre-line break-words text-xs text-rose-800"><strong>{{ __('reviews.moderation.private_note') }}:</strong> {{ $adminItem->activeRestriction['private_note'] }}</p>
                                    @endif
                                </div>
