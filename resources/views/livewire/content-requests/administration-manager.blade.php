            ...
            @foreach ($requests as $request)
                <div wire:key="admin-request-{{ $request->publicId }}" class="rounded-panel border border-slate-200 bg-white p-4 shadow-panel sm:p-5">
                    ...
                    <div class="mt-4 grid gap-4 border-t border-slate-100 pt-4 lg:grid-cols-2">
                        <div class="space-y-3">
                            @if (count($statusActionOptions[$request->id] ?? []) > 0)
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <select id="admin-status-{{ $request->id }}" wire:model="desiredStatuses.{{ $request->id }}" class="mt-2 min-h-11 w-full rounded-control border border-slate-300 px-3">
                                            @foreach ($statusActionOptions[$request->id] as $option)
                                                <option value="{{ $option['value'] }}">{{ $option['label'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @if ($adminCapabilities[$request->id]['reject'] ?? false)
                                        <div>
                                ...
                                @if ($adminCapabilities[$request->id]['complete'] ?? false)
                        ...
                        <div class="space-y-3">
                            ...
                            @if ($adminCapabilities[$request->id]['clarify'] ?? false)
                                <div>
                            ...
                            @if ($adminCapabilities[$request->id]['merge'] ?? false)
                            ...
                            @endif
                            @if ($adminCapabilities[$request->id]['handoff'] ?? false)
