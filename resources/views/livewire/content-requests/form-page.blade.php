            ...
            @if ($type !== 'serial' && $type !== 'other_content_request')
                <x-ui.panel :title="__('requests.form.target_details')" icon="fa-solid fa-layer-group">
                    ...
                    @if ($catalogTitleId !== '' && count($seasons) > 0)
                        <div class="grid gap-4 md:grid-cols-2"><div><label for="request-season" class="block text-sm font-bold text-slate-700">{{ __('requests.fields.season') }}</label><select id="request-season" wire:model.live="seasonId" class="mt-2 min-h-11 w-full rounded-control border border-slate-300 px-3 py-2"><option value="">{{ __('requests.fields.whole_serial') }}</option>@foreach ($seasons as $season)<option value="{{ $season->id }}">{{ __('requests.fields.season_number_value', ['number' => $season->number]) }}</option>@endforeach</select></div>
                        @if ($type === 'season')<x-form.field :label="__('requests.fields.season_number')" for="request-season-number" type="number" wire:model="seasonNumber" required />@endif
                        @if ($type === 'episode')<x-form.field :label="__('requests.fields.episode_number')" for="request-episode-number" type="number" wire:model="episodeNumber" required />@endif</div>
                    @elseif ($catalogTitleId !== '' && $type === 'season')
                        <x-form.field :label="__('requests.fields.season_number')" for="request-season-number-empty" type="number" wire:model="seasonNumber" required />
                    @endif
                </x-ui.panel>
            @endif

            @if ($type === 'translation' || $type === 'subtitles')
