<div class="relative" data-catalog-directory="{{ $definition->key }}">
    ...
    <header class="border-b border-slate-200 px-1 py-6 sm:px-2 lg:py-8">
        <nav aria-label="{{ __('catalog.directories.breadcrumbs') }}" class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm font-semibold text-slate-600">
            <a href="{{ route('home') }}" class="min-h-11 py-3 hover:text-emerald-800 hover:underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200">{{ __('catalog.navigation.home') }}</a>
            <x-ui.icon name="fa-solid fa-chevron-right text-[10px] text-slate-300" />
            <a href="{{ route('titles.index') }}" class="min-h-11 py-3 hover:text-emerald-800 hover:underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200">{{ __('catalog.navigation.all_titles') }}</a>
            <x-ui.icon name="fa-solid fa-chevron-right text-[10px] text-slate-300" />
        ...
        <div class="mt-4 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div class="max-w-4xl">
                <div class="flex items-center gap-3 text-emerald-700">
                    ...
                    <span class="text-sm font-semibold">{{ __('catalog.directories.label') }}</span>
                </div>
                <h1 class="mt-3 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">{{ $definition->title }}</h1>
            ...
            <dl class="grid grid-cols-2 gap-3 sm:max-w-lg xl:min-w-[340px]">
                <div class="rounded-control bg-slate-50 px-4 py-3">
                    <dt class="text-xs font-bold text-slate-600">{{ __('catalog.directories.total_values') }}</dt>
                    <dd class="mt-1 text-xl font-semibold text-slate-900">{{ $totalValuesLabel }}</dd>
                </div>
                <div class="rounded-control bg-emerald-50 px-4 py-3">
                    <dt class="text-xs font-bold text-emerald-700">{{ __('catalog.directories.total_titles') }}</dt>
                    <dd class="mt-1 text-xl font-semibold text-emerald-900">{{ $totalTitlesLabel }}</dd>
                </div>
            </dl>
        </div>
    </header>

    <section aria-labelledby="directory-controls" class="mt-5 rounded-panel border border-slate-200 bg-white p-4 sm:p-6">
        ...
        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(210px,auto)] lg:items-end">
            <div>
                ...
                <div class="flex min-w-0 items-center rounded-control border border-slate-300 bg-white focus-within:border-emerald-600 focus-within:ring-4 focus-within:ring-emerald-100">
                    ...
                    >
                    @if ($search !== '')
                        <button type="button" wire:click="clearSearch" class="inline-flex min-h-11 min-w-11 items-center justify-center text-slate-600 hover:text-emerald-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200" aria-label="{{ __('catalog.directories.clear_search') }}">
            ...
            <div data-directory-alphabet-groups class="mt-5" aria-label="{{ __('catalog.directories.alphabet') }}">
                ...
                <div data-directory-alphabet-symbols class="mt-2 flex flex-wrap items-center gap-1.5">
                    <button type="button" wire:click="setLetter('')" @class([
                        ...
                        'bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-800' => $letter !== '',
                    ...
                    @foreach ($letterGroups['symbols'] as $availableLetter)
                        <button type="button" wire:key="directory-letter-{{ $availableLetter }}" wire:click="setLetter(@js($availableLetter))" @class([
                            ...
                            'bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-800' => $letter !== $availableLetter,
                ...
                @foreach (['cyrillic', 'latin'] as $group)
                    @if ($letterGroups[$group] !== [])
                        <div data-directory-alphabet-group="{{ $group }}" class="mt-3 grid gap-2 sm:grid-cols-[6.5rem_minmax(0,1fr)] sm:items-start">
                            <span class="py-3 text-xs font-bold text-slate-600">{{ __("catalog.catalog.alphabet.{$group}") }}</span>
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    @foreach ($decades as $availableDecade)
                        <button type="button" wire:key="directory-decade-{{ $availableDecade }}" wire:click="setDecade({{ $availableDecade }})" @class([
                            ...
                            'bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-800' => $decade !== $availableDecade,
        ...
        @if ($search !== '' || $letter !== '' || $decade !== null || $sort !== 'name_asc')
            <button type="button" wire:click="resetDirectoryFilters" class="mt-5 inline-flex min-h-11 items-center gap-2 px-1 py-2 text-sm font-bold text-slate-600 hover:text-emerald-800 hover:underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200">
    ...
    @island(name: 'directory-pagination', always: true, with: $this->paginationPage)
    <x-ui.pagination-region name="directory-results" class="mt-6">
    <section aria-labelledby="directory-results">
        <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
                <h2 id="directory-results" class="text-2xl font-semibold text-slate-900">{{ __('catalog.directories.results') }}</h2>
                <p class="mt-1 text-sm text-slate-600">{{ $foundValuesLabel }}</p>
            </div>
            @if ($items->lastPage() > 1)
                <p class="text-sm font-semibold text-slate-600">{{ $pageLabel }}</p>
            @endif
        </div>

        @if ($items->isEmpty())
            <div class="mt-4 border-t border-slate-200 px-5 py-12 text-center" role="status">
                <x-ui.icon name="fa-solid fa-folder-open text-3xl text-slate-300" />
                <h3 class="mt-4 text-lg font-semibold text-slate-900">{{ __('catalog.directories.empty') }}</h3>
            ...
            <div class="mt-4 space-y-8">
                @foreach ($itemsByDecade as $itemDecade => $decadeItems)
                    <section aria-labelledby="decade-{{ $itemDecade }}">
                        <h3 id="decade-{{ $itemDecade }}" class="text-lg font-semibold text-slate-900">{{ __('catalog.directories.decade', ['decade' => $itemDecade]) }}</h3>
        ...
        @if ($items->hasPages())
            <nav class="mt-7" aria-label="{{ __('catalog.directories.pagination') }}">
                {{ $items->onEachSide(1)->links(data: ['region' => 'directory-results']) }}
            </nav>
        @endif
    </section>
