...
>
    ...
    <x-ui.panel :title="__('catalog.importer.queue_status')" :subtitle="__('catalog.importer.queue_status_description')" icon="fa-solid fa-server">
        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ($queueStatus['metrics'] as $metric)
                <div wire:key="import-queue-{{ $metric['key'] }}" class="rounded-control bg-slate-50 p-3">
        ...
        <div class="mt-3 grid gap-2 lg:grid-cols-3">
            @foreach ([$queueStatus['phase'], $queueStatus['transport'], $queueStatus['worker']] as $state)
                <div wire:key="import-queue-state-{{ $state['code'] }}" class="flex min-h-11 items-center gap-2 rounded-control bg-slate-50 px-3 py-2 text-sm font-semibold {{ $state['tone'] }}">
                    <x-ui.icon name="{{ $state['icon'] }}" />
        ...
        <div class="mt-3 grid gap-2 text-sm font-semibold text-slate-600 lg:grid-cols-2">
            <div class="flex items-start gap-2">
                <x-ui.icon name="fa-solid fa-layer-group text-sky-700" align="start" />
                <span>{{ $queueStatus['durable_progress'] }}</span>
            </div>
            <div class="flex items-start gap-2">
                <x-ui.icon name="fa-solid fa-paper-plane text-violet-700" align="start" />
                <span>{{ $queueStatus['dispatch'] }}</span>
            </div>
            <div class="flex items-start gap-2">
                <x-ui.icon name="fa-solid fa-chart-line text-emerald-700" align="start" />
                <span>{{ $queueStatus['last_progress'] }}</span>
            </div>
            <div class="flex items-start gap-2">
                <x-ui.icon name="fa-solid fa-heart-pulse text-amber-700" align="start" />
                <span>{{ $queueStatus['worker_heartbeat'] }}</span>
            </div>
            <div class="flex items-start gap-2">
                <x-ui.icon name="fa-solid fa-hourglass-end text-slate-600" align="start" />
                <span>{{ $queueStatus['claim_expiry'] }}</span>
            </div>
            @if ($queueStatus['finalization_stage'])
                <div class="flex items-start gap-2">
                    <x-ui.icon name="fa-solid fa-flag-checkered text-sky-700" align="start" />
                    <span>{{ $queueStatus['finalization_stage'] }}</span>
                </div>
            @endif
        </div>

        @if ($queueStatus['stale_reason'] || $queueStatus['terminal_reason'])
            <div role="status" class="mt-3 flex flex-col gap-1 rounded-control bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
                @if ($queueStatus['stale_reason'])
                    <span>{{ $queueStatus['stale_reason'] }}</span>
                @endif
                @if ($queueStatus['terminal_reason'])
                    <span>{{ $queueStatus['terminal_reason'] }}</span>
                @endif
            </div>
        @endif
    </x-ui.panel>
    ...
    <x-ui.panel :title="__('catalog.importer.file_size_backlog')" :subtitle="__('catalog.importer.file_size_backlog_description')" icon="fa-solid fa-hard-drive">
        ...
        <p @class([
            ...
            'text-amber-700' => $mediaSizeBacklog['stale'],
            'text-slate-500' => ! $mediaSizeBacklog['stale'],
        ])>
            <x-ui.icon :name="$mediaSizeBacklog['stale'] ? 'fa-solid fa-triangle-exclamation text-amber-600' : 'fa-solid fa-camera text-slate-400'" />
            <span>{{ $mediaSizeBacklog['captured_at'] }}</span>
            @if ($mediaSizeBacklog['stale'])
                <span>· {{ $mediaSizeBacklog['stale_notice'] }}</span>
            @endif
        </p>
    </x-ui.panel>
