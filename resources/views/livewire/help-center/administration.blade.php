<div class="space-y-5">
    ...
    @elseif ($tab === 'feedback')
        <section class="overflow-x-auto rounded-panel border border-slate-200 bg-white p-5 shadow-panel"><h2 class="text-xl font-black">{{ __('help.admin.feedback') }}</h2><table class="mt-4 min-w-full text-left text-sm"><thead><tr class="border-b border-slate-200"><th class="p-3">{{ __('help.admin.fields.article') }}</th><th class="p-3">{{ __('help.admin.fields.locale') }}</th><th class="p-3">{{ __('help.admin.fields.value') }}</th><th class="p-3">{{ __('help.admin.fields.count') }}</th></tr></thead><tbody>@forelse ($feedbackAggregates as $aggregate)<tr class="border-b border-slate-100"><td class="p-3 font-bold">{{ $aggregate->translation?->title }}</td><td class="p-3">{{ $aggregate->translation?->locale }}</td><td class="p-3">{{ $aggregate->value->label() }}</td><td class="p-3">{{ $aggregate->responses_count }}</td></tr>@empty<tr><td colspan="4" class="p-5 text-slate-600">{{ __('help.states.no_feedback') }}</td></tr>@endforelse</tbody></table><div class="mt-3">{{ $feedbackAggregates->links() }}</div></section>
    @else
