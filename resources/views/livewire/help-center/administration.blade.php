<div class="space-y-5">
                    ...
                    @forelse ($articles as $item)
                        <li>
                            <button type="button" wire:click="editArticle({{ $item->id }}, '{{ $localeFilter }}')" @class(['w-full rounded-control border p-3 text-left', 'border-emerald-300 bg-emerald-50' => $articleId === $item->id, 'border-slate-200 hover:bg-slate-50' => $articleId !== $item->id])>
                                ...
                                <span class="mt-1 flex flex-wrap gap-2 text-xs text-slate-500"><span>{{ $item->status->label() }}</span><span>{{ $item->category?->code }}</span><span>{{ __('help.admin.counters.revisions', ['count' => $item->revisions_count]) }}</span><span>{{ __('help.admin.counters.feedback', ['count' => $item->feedback_count]) }}</span><span>{{ __('help.admin.counters.reports', ['count' => $item->open_reports_count]) }}</span></span>
                            </button>
                        </li>
                    @empty
            ...
            <section class="min-w-0 rounded-panel border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
                ...
                @if ($selectedArticle !== null)
                    <div class="mt-6 border-t border-slate-200 pt-5">
                        <h3 class="font-black text-slate-900">{{ __('help.admin.fields.status') }}: {{ $selectedArticle->status->label() }}</h3>
                        <div class="mt-3 flex flex-wrap gap-2">
                            @foreach ($selectedArticle->status->allowedTransitions() as $target)
                            ...
                            @if ($replacementCode !== '' && $selectedArticle->status->value !== 'archived')
    ...
    @elseif ($tab === 'feedback')
        <section class="overflow-hidden rounded-panel border border-slate-200 bg-white p-5 shadow-panel"><h2 class="text-xl font-black">{{ __('help.admin.feedback') }}</h2><table class="mt-4 w-full table-fixed text-left text-sm"><thead><tr class="border-b border-slate-200"><th class="p-3">{{ __('help.admin.fields.article') }}</th><th class="p-3">{{ __('help.admin.fields.locale') }}</th><th class="p-3">{{ __('help.admin.fields.value') }}</th><th class="p-3">{{ __('help.admin.fields.count') }}</th></tr></thead><tbody>@forelse ($feedbackAggregates as $aggregate)<tr class="border-b border-slate-100"><td class="break-words p-3 font-bold">{{ $aggregate->translation?->title }}</td><td class="break-words p-3">{{ $aggregate->translation?->locale }}</td><td class="break-words p-3">{{ $aggregate->value->label() }}</td><td class="break-words p-3">{{ $aggregate->responses_count }}</td></tr>@empty<tr><td colspan="4" class="p-5 text-slate-600">{{ __('help.states.no_feedback') }}</td></tr>@endforelse</tbody></table><div class="mt-3">{{ $feedbackAggregates->links() }}</div></section>
    @else
        <section class="space-y-4"><h2 class="text-xl font-black text-slate-900">{{ __('help.admin.reports') }}</h2>@forelse ($reports as $report)<article class="rounded-panel border border-slate-200 bg-white p-5 shadow-panel"><div class="flex flex-wrap items-start justify-between gap-3"><div><h3 class="font-black text-slate-900">{{ $report->translation?->title ?? $report->article?->code }}</h3><p class="mt-1 text-xs text-slate-500">{{ $report->locale }} · {{ $report->reason->label() }} · {{ $report->created_at?->format('d.m.Y H:i') }}</p></div><span class="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-900">{{ __('help.admin.report_statuses.'.$report->status) }}</span></div>@if ($report->details)<p class="mt-3 whitespace-pre-line text-sm leading-6 text-slate-700">{{ $report->details }}</p>@endif<div class="mt-4 flex flex-wrap gap-2"><input wire:model="reportPrivateNotes.{{ $report->id }}" aria-label="{{ __('help.admin.fields.private_note') }}" maxlength="2000" placeholder="{{ __('help.admin.fields.private_note') }}" class="min-h-11 min-w-0 flex-1 rounded-control border border-slate-300 px-3"><button type="button" wire:click="reviewReport({{ $report->id }}, 'reviewed')" wire:loading.attr="disabled" wire:target="reviewReport" class="min-h-11 rounded-control bg-emerald-700 px-4 py-2 text-sm font-black text-white">{{ __('help.admin.reviewed') }}</button><button type="button" wire:click="reviewReport({{ $report->id }}, 'dismissed')" wire:loading.attr="disabled" wire:target="reviewReport" class="min-h-11 rounded-control bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{{ __('help.admin.dismiss') }}</button></div></article>@empty<div class="rounded-panel border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-600">{{ __('help.admin.empty') }}</div>@endforelse<div>{{ $reports->links() }}</div></section>
    @endif
</div>
