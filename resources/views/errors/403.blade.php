    ...
    <div class="mx-auto max-w-2xl">
        <section class="rounded-panel border border-slate-200 bg-white p-4 shadow-panel sm:p-6" aria-labelledby="forbidden-title">
            <div class="flex items-start gap-3">
                ...
                <div class="min-w-0">
                    <h1 id="forbidden-title" class="break-words text-2xl font-black tracking-tight text-slate-800 sm:text-3xl">{{ $title ?? 'Доступ запрещён' }}</h1>
                    <p class="mt-2 text-sm leading-6 text-slate-600">{{ $message ?? 'У вас нет доступа к этой странице.' }}</p>
                </div>
            </div>
