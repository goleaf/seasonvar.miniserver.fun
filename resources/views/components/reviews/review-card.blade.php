...
<article
    ...
    tabindex="-1"
    class="scroll-mt-24 rounded-panel border bg-white p-4 shadow-panel outline-none transition focus-visible:ring-2 focus-visible:ring-emerald-500 sm:p-5 {{ $review->isHighlighted ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-200' }}"
>
    ...
    @if (! $review->isDeleted || $context === 'admin')
        ...
        @if ($review->bodyHidden)
            <div id="review-body-{{ $review->id }}" class="mt-4 rounded-control border border-violet-200 bg-violet-50 p-4">
        ...
        @elseif ($review->body !== null)
            <div id="review-body-{{ $review->id }}" class="mt-4 whitespace-pre-line break-words text-[15px] leading-7 text-slate-700">{{ $review->body }}</div>
    ...
    <footer class="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-bold">
            ...
            @if ($review->canModerate)<a href="{{ route('admin.reviews', ['review' => $review->id]) }}" class="inline-flex min-h-11 items-center text-slate-600 hover:text-slate-900">{{ __('reviews.actions.moderate') }}</a>@endif
